...
def _build_input_content(
    ...
    if test_plan_entries:
        ...
        lines.append(
            ...
            "**Every test body MUST call `await page.goto(...)` at least "
            "once before its `expect(...)` assertions.** A Playwright test "
            "without navigation leaves the tab on `about:blank`, producing "
            "empty/white screenshots in the per-attempt artifact gallery — "
            "useless for review and forensic debugging. **NEVER use "
            "`readFileSync`, `fs.readFile`, or any source-file reading in a "
            "Playwright `tests/e2e/*.spec.ts` test.** Source-content "
            "assertions like `expect(source).toContain('<ThemeProvider')` are "
            "weak (would pass even inside an `if (false)` branch) and never "
            "navigate, so they generate blank artifacts. Test the rendered "
            "DOM behaviour instead: navigate to the route, locate by role "
            "or testid, assert what the user actually sees. Source-level or "
            "type-level checks belong in a unit test runner (vitest/jest), "
            "not in `tests/e2e/`. "
