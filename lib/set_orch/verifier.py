...
#: Candidate locations of the framework's spec-verify rule file, in resolution
#: order. The deploy prefixes core rules with ``set-`` when it copies them into a
#: consumer, and set-core itself is skipped by that deploy — so the same document
#: sits under three different names depending on where the gate is running. The
#: path is RESOLVED at call time rather than named in the prompt, because naming
#: one of them would be a guess that is wrong in two of the three cases.
_VERIFY_RULES_CANDIDATES = (
    ".claude/rules/set-spec-verify-gate.md",   # deployed into a consumer
    ".claude/rules/spec-verify-gate.md",       # hand-placed / unprefixed
    "templates/core/rules/spec-verify-gate.md",  # set-core's own tree (not self-deployed)
)


def _resolve_verify_rules_path(wt_path: str) -> str:
    """Return the worktree-relative path of the spec-verify rule file, or "".

    The generated ``/opsx:verify`` skill is rewritten wholesale by every
    ``openspec update``, so the framework's own verification requirements
    (the two sentinels, traceability, scope/overshoot, the per-change verify
    hook) live in a framework-owned rule file instead. Returning "" when none
    is present is deliberate: the prompt still teaches the sentinels inline,
    so a missing rule file degrades the gate's depth, never its verdict.
    """
    for rel in _VERIFY_RULES_CANDIDATES:
        if os.path.isfile(os.path.join(wt_path, rel)):
            return rel
    logger.debug(
        "Gate[spec-verify]: no spec-verify rule file under %s (looked for %s)",
        wt_path, ", ".join(_VERIFY_RULES_CANDIDATES),
    )
    return ""
...
def _execute_spec_verify_gate(
    ...
    # The stock /opsx:verify skill is regenerated by every `openspec update`, so
    # the framework's own checks live in a rule file the CLI never touches. Named
    # here only when it actually exists — a prompt that orders a read of a missing
    # file spends a turn on an error and teaches nothing.
    _rules_rel = _resolve_verify_rules_path(wt_path)
    _rules_line = (
        f"FIRST read `{_rules_rel}` and follow it — it carries the framework's\n"
        f"verification requirements, which the stock skill does not.\n\n"
        if _rules_rel else ""
    )
