...
cmd_status() {
    ...
    while IFS=$'\t' read -r name change_status tokens test_res review_res g_ms g_rtok g_rcnt redisp_cnt; do
        ...
        if [[ -n "$wt_path" && -f "$wt_path/.set/loop-state.json" ]]; then
            local iter max_iter
            iter=$(jq -r '.current_iteration // 0' "$wt_path/.set/loop-state.json" 2>/dev/null)
            max_iter=$(jq -r '.max_iterations // "?"' "$wt_path/.set/loop-state.json" 2>/dev/null)
    ...
    if [[ "$waiting_human" -gt 0 ]]; then
        ...
        jq -r '.changes[] | select(.status == "waiting:human") | .name' "$STATE_FILENAME" | while read -r wh_name; do
            ...
            local wh_summary=""
            if [[ -n "$wh_wt" && -f "$wh_wt/.set/loop-state.json" ]]; then
