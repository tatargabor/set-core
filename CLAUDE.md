...
3. **Re-arm BOTH watches — this is the step that actually gets forgotten.** A compact, a
   `/clear` or a session restart silently kills them, and a dead watch is indistinguishable
   from a quiet peer, so the work stops without anyone noticing:
   - a **Monitor** on the other side's file size (`persistent: true`) — how you learn about
     new entries without polling by hand;
   - a **CronCreate** catch-up every ~10 minutes as the fallback for when the Monitor itself
     dies. Its prompt: read the last `W#`/peer entry, check whether you have already answered
     it, do the work and reply if not, restart the Monitor if it is gone, and **say nothing at
     all when there is nothing to do** — a fallback that chatters gets muted. Pick a period
     that does not coincide with the peer's (they run one too); cron jobs are session-only and
     expire after 7 days.
...
- **That the Monitor and the catch-up cron must be re-armed after the compact.** They do not
  survive it. Nothing reports their death, and a peer waiting on an answer looks exactly like
  a peer with nothing to say — so re-arm first, before picking the work back up.
