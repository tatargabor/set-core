...
## IN SCOPE
- New `set-design-hygiene` CLI generates per-project hygiene checklist.
- 9 quality rules detecting common design-source antipatterns.
- Markdown checklist output with severity tiers (CRITICAL / WARN / INFO).
- Optional `--with-hygiene` flag on `set-design-import` for combined workflow.

## OUT OF SCOPE
- Automatic correction of design source files (operator fixes manually in design source repo).
- Fixing the design source's git provider (separate location, possibly different auth).
- Pixel-level visual quality (covered by fidelity gate's pixel-diff phase, separate change).
...
The scanner SHALL output a markdown file structured as:
```markdown
# Design Source Hygiene — <project-id>
Auto-generated by `set-design-hygiene` from `<source>-export/` on <ISO-date>.
N findings across 3 severity tiers.
