...
- [x] 1.1 In `lib/wt_orch/verifier.py`, remove the `verify_retry_count += 1` and `update_change_field(..., "verify_retry_count", ...)` calls from the build-failure retry path (around line 1306-1308). Keep the `status = "verify-failed"` and `resume_change()` dispatch — just don't increment the counter.
...
- [x] 2.1 In `lib/wt_orch/dispatcher.py`, add `_AUTO_RESOLVE_PREFIXES` set containing `".claude/"` alongside the existing `_CORE_GENERATED_FILE_PATTERNS`.
...
- [x] 3.1 In `bin/wt-sentinel`, after the `flock -n 9` failure block (line 73-76), add PID validation: read PID from `sentinel.pid`, check `kill -0 "$existing_pid" 2>/dev/null`, if dead then `rm -f "$LOCK_FILE"`, re-open fd 9, retry flock.
...
- [x] 4.1 In `lib/wt_orch/engine.py` `_poll_active_changes()`, add a check before calling `poll_change()`: if `change.status == "verifying"` and the change has stored gate results where all blocking gates are "pass" or "skipped", call the merge path directly instead of polling.
