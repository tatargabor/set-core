...
- [x] 10.13 After `IssueRegistry.register()` in `escalate_change_to_fix_iss()`, set the returned `Issue.change_name = fix_iss_name` and call `registry.save()` so the issue is linked to the ALREADY-created fix-iss change. In `IssueManager._process` `case IssueState.NEW`, if `issue.source.startswith("circuit-breaker:")` transition directly to `DIAGNOSED` instead of spawning `/opsx:ff`. Without both: when the issue is processed the investigator derives a DIFFERENT `change_name` from the error_summary slug, producing a ghost duplicate fix-iss change in `state.changes` with a mangled name (observed as `fix-iss-001-catalog-listings-and-homepage` alongside the real `fix-iss-001-catalog-listings-and`), which eats a dispatch slot and confuses the plan graph [REQ: auto-escalate-to-fix-iss-on-retry-budget-exhaustion]
