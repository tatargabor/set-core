## IN SCOPE
- Reading the last turns of a session the framework has RECORDED but nothing is running on
- Saying which entries can be read and which cannot, with the reason
- Rendering that content at request time only

## OUT OF SCOPE
- Resuming, starting or otherwise touching the session (that is `agent-fleet-restore`)
- The live agent's log view, which already exists and is keyed on a pid
- Searching, filtering or summarising transcript content
- Storing any of it — see the persistence requirement below
