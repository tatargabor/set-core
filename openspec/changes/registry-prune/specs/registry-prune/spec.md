## IN SCOPE
- Deregistering registry entries whose directory no longer exists
- Distinguishing "directory deleted" from "filesystem unavailable"
- Pruning orphaned git worktree records in registered projects
- Archiving registry entries whose directory still exists, reversibly
- The loss-free guarantee and how it is proven
- Preview mode, confirmation, and the pre-write backup

## OUT OF SCOPE
- Deleting any file, directory, worktree or branch (never, under any flag)
- Worktree GC for merged changes (`set-cleanup`, unchanged)
- Operator-initiated single-project removal (`set-project remove`, unchanged)
- How archived entries are rendered (see `manager-api` and the overview)
