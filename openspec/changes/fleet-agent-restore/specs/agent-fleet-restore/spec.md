## IN SCOPE
- A per-project act that brings back every agent in that project's recorded list.
- Starting an agent and resuming its recorded session, through the service that owns agent lifetimes.
- A separate, stated outcome for every entry: started, skipped and why, or failed and why.
- Refusing — never forking — a session a live process is already bound to.
- Reporting a restore that partly succeeded as partly succeeded.

## OUT OF SCOPE
- Choosing a subset of the list. Restore brings back the whole recorded list; a per-entry selection is a different act.
- Writing the record. That is `agent-fleet-snapshot`.
- Automatic restore on boot. The act is chosen by a person, per project.
- Restoring a session that has no transcript — there is nothing to resume.
- Any deployment, promotion or release action as a consequence of restoring.
