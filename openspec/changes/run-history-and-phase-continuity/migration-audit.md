...
- [ ] `lib/set_orch/paths.py` — add `LineagePaths` class; extend `SetRuntime` or wrap it. Support `lineage_id` parameter. Expose project-level + worktree-level path properties. Keep legacy `SetRuntime` unchanged for runtime-shared paths under `~/.local/share/set-core/`.
...
- [ ] `lib/set_orch/planner.py` — plan WRITE, domains WRITE, digest_dir reads (multiple sites)
...
- [ ] `lib/set_orch/recovery.py` — state READ
- [ ] `lib/set_orch/config.py` — config.yaml READ, specs dir fallback
...
- [ ] `lib/set_orch/notifications.py` — state READ
- [ ] `lib/set_orch/cross_change.py` — state READ
- [ ] `lib/set_orch/loop_prompt.py` — reflection READ
- [ ] `lib/set_orch/loop_state.py` — review-learnings path reference
- [ ] `lib/set_orch/cli.py` — default path args (11 refs)
- [ ] `lib/set_orch/cli_entry.py` — serve-time path wiring (verify no hardcoded)
...
- [ ] `lib/set_orch/supervisor/triggers.py` — state consume
- [ ] `lib/set_orch/supervisor/canary.py` — state consume
- [ ] `lib/set_orch/supervisor/service.py` — spec-path write into status
- [ ] `lib/set_orch/sentinel/status.py` — status.json writes via archive_and_write
...
- [ ] `lib/set_orch/api/activity_detail.py` — activity-detail-v2-*.jsonl path
- [ ] `lib/set_orch/api/sessions.py` — session file discovery (partly worktree-based — confirm)
- [ ] `lib/set_orch/api/actions.py` — config.yaml, spec path handling
- [ ] `lib/set_orch/api/sentinel.py` — spec-path handling on start
- [ ] `lib/set_orch/api/lifecycle.py` — IssueRegistry(project_path) — confirm path
- [ ] `lib/set_orch/api/media.py` — artifact scan
- [ ] `lib/set_orch/api/_sentinel_orch.py` — sentinel events path

### 2.d — Issues subsystem (3 files)

- [ ] `lib/set_orch/issues/registry.py` — `.set/issues/registry.json` path
...
- [ ] `lib/set_orch/manager/service.py` — IssueRegistry + state access
- [ ] `lib/set_orch/profile_loader.py` — default `output_path="docs/spec.md"` — review; may need lineage-aware default
- [ ] `lib/set_orch/profile_types.py` — review-learnings project-level path
- [ ] `lib/set_orch/subprocess_utils.py` — PLAN_FILENAME env var handling
- [ ] `lib/set_orch/state.py` — state load/save centralisation (review if it hardcodes paths)
- [ ] `lib/set_orch/archive.py` — archive_and_write helper (generic; confirm no project-level assumptions)
- [ ] `lib/set_orch/agent/messaging.py` — messages dir (if applicable)

### 2.f — Legacy (4 files — deprecate or migrate)

- [ ] `lib/set_orch/_api_old.py` — 17 refs (state, archive, report, events); decide: migrate OR formally deprecate in this change
- [ ] `lib/set_orch/archive.py` (legacy) — confirm no hardcoded orchestration paths
- [ ] `lib/set_orch/sentinel/findings.py` — findings path
- [ ] `lib/set_orch/sentinel/orchestrator.py` — state consume

### 2.g — Migrations + utilities (1 file)

- [ ] `scripts/migrate-review-learnings.py` — review-findings path
...
- [ ] `bin/set-orchestrate` — state + plan path derivation from env (multiple sites)
- [ ] `bin/set-sentinel` — spec argument → state
- [ ] `bin/set-status` — reads state
- [ ] `bin/set-web` — serve wiring
- [ ] `bin/set-manager` — reads state
- [ ] `bin/set-close` — worktree cleanup (extend with `--purge` + history)
- [ ] `bin/set-new` — worktree init (no orch-path refs expected, confirm)
- [ ] `bin/set-merge` — invokes merger; check for hardcoded state paths
- [ ] `bin/set-harvest` — reads archive + state across projects
- [ ] `bin/set-memory` — memory paths (cross-project, unrelated; confirm no orchestration-state refs)
- [ ] `bin/set-project` — init path wiring
- [ ] `bin/set-common.sh` — shared helpers (most likely the right centralisation point for Bash)
...
- [ ] `lib/orchestration/merger.sh` (if still referenced)
- [ ] `lib/orchestration/verifier.sh` (if still referenced)
- [ ] `lib/loop/engine.sh` — reflection READ
- [ ] `lib/loop/state.sh` — state READ
...
- [ ] `web/src/pages/Dashboard.tsx` — confirm every data-fetching useEffect uses selected lineage
...
- [ ] `tests/orchestrator/test-orchestrate-integration.sh` — 84 refs. Update to construct paths via a test helper that mirrors `LineagePaths`.
- [ ] `tests/unit/test_state.py`
- [ ] `tests/unit/test_state_journal.py`
- [ ] `tests/unit/test_loop_state.py`
- [ ] `tests/unit/test_worktree_harvest.py`
- [ ] `tests/unit/test_archive_completed_to_jsonl.py` — already added; confirm it uses `LineagePaths`
- [ ] `tests/unit/test_load_archived_changes.py` — already added; confirm it uses `LineagePaths`
- [ ] `tests/unit/test_archive.py`
- [ ] `tests/unit/test_issue_state_machine.py`
- [ ] `tests/unit/helpers.sh` — shared test helper; extend with `LineagePaths`-equivalent bash helper
- [ ] `tests/unit/test_orch_state.sh`
