...
- [ ] 1.1 `lib/set_orch/usage/accounts.py`: `purge_accounts(directory=None, targets) -> dict`
...
- [ ] 1.2 `kind: glm` is refused with an answer naming `providers.json` as the place the
      credential is managed; `providers.json` untouched
- [ ] 1.3 `tests/unit/test_usage_accounts.py` (or a sibling `test_usage_purge.py`): web
...
- [ ] 2.1 `lib/set_orch/api/usage.py`: `POST /api/usage/accounts/purge`, body
...
- [ ] 2.2 Extend `tests/unit/test_usage_api.py`: route registered before project
...
- [ ] 3.1 `web/src/components/FleetUsageStrip.tsx`: purge action on the "did not answer"
...
- [ ] 3.2 Extend `web/tests/unit/fleetUsageStrip.test.tsx`: button present when something
...
- [ ] 3.3 `web/src/lib/fleetUsageBars.ts`: expose the silent accounts' kind/name pairs for
...
- [ ] 4.2 vitest: the touched suites
- [ ] 4.3 Visual check in the browser: button appears on the live strip's "did not answer"
...
- [ ] 4.4 Commit with the spec artifacts
