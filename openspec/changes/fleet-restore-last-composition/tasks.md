...
- [x] 6.5 LOOKED at it, 2026-08-26, on the running dashboard. A project with **2 open and 49 recorded** reads `All 2 already running` + `⌄ 47 more recorded here, not open`; the disclosure lists all 47, each with its age, the blocked ones greyed with their reason (`running now — a resume would fork its conversation`, `no session id was ever recorded`, `no transcript on disk for session …`); ticking one produced `⟲ Restore 1 selected`. **The look found a defect the tests could not**: 47 rows pushed the entire agent grid off the screen, so the list is now capped (`max-h-64 overflow-y-auto`) and the tiles stay visible — re-checked in the browser after the fix [REQ: the-surface-offers-restore-per-project-and-shows-what-happened]
