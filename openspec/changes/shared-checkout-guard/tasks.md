...
## 2. Recording who staged what

- [ ] 2.1 Read `session_id` from the payload the way `bin/set-hook-memory:50` already does; state in a comment what happens when it is absent [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign]
- [ ] 2.2 On an allowed `git add`, append the named pathspecs to `/tmp/set-checkout-guard-<session_id>.json`, keyed by repository root so one session working in two trees does not cross-claim [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign]
- [ ] 2.3 Ownership match: an index path is the session's own if it EQUALS a recorded pathspec, sits BENEATH a recorded directory, or fnmatches a recorded glob — all three, because that is what `git add` accepts [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign]
- [ ] 2.4 Second condition (design D3): a path claimed by ANOTHER session's record is foreign even when it matches our own pattern, so a broad `git add <dir>` cannot swallow a neighbour's staged file [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign]
- [ ] 2.5 A staged path attributable to no observed command is FOREIGN, not ours — assert this direction in a test, because the opposite default reproduces the defect exactly [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign]
...
- [ ] AC-10: WHEN a session stages a path and commits in a separate command THEN the guard recognises that path as the session's own [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign, scenario: the-sessions-staging-is-remembered-across-commands]
- [ ] AC-11: WHEN the index holds a path no observed command of the running session staged THEN it is treated as foreign and is not assumed to be the session's own [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign, scenario: a-path-staged-by-nobody-the-guard-saw-is-foreign]
- [ ] AC-12: WHEN a session that has staged nothing runs a pathspec-less `git commit` and the index is not empty THEN the command is refused [REQ: ownership-is-recorded-and-what-cannot-be-attributed-is-foreign, scenario: a-session-that-staged-nothing-does-not-own-a-populated-index]
- [ ] AC-13: WHEN the guard refuses a command THEN the index, the working tree and the stash list are exactly as they were, and foreign staged entries are untouched [REQ: the-guard-refuses-and-changes-nothing-itself, scenario: a-refusal-leaves-everything-where-it-was]
- [ ] AC-14: WHEN the guard finds a foreign staged path THEN it does not unstage it [REQ: the-guard-refuses-and-changes-nothing-itself, scenario: the-guard-does-not-unstage-on-the-sessions-behalf]
- [ ] AC-15: WHEN an agent in its own git worktree stages its work and commits with no pathspec THEN the command is allowed [REQ: the-guard-is-silent-where-the-hazard-does-not-exist, scenario: a-dedicated-worktree-is-not-policed]
- [ ] AC-16: WHEN the command runs somewhere that is not a git repository THEN the guard allows it rather than erroring [REQ: the-guard-is-silent-where-the-hazard-does-not-exist, scenario: a-command-outside-a-repository-passes-through]
- [ ] AC-17: WHEN a session runs a command that is not a guarded git verb THEN the guard allows it without examining the index [REQ: the-guard-is-silent-where-the-hazard-does-not-exist, scenario: a-non-git-command-is-not-inspected]
