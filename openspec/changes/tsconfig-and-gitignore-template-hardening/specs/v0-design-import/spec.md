## IN SCOPE
- `set-design-import` CLI responsibilities when running against a scaffolded web consumer project
- Idempotent patching of the project's `tsconfig.json` exclude list to cover v0-export variants and backup directories
- Logging surface for "what was added" versus "already up to date"

## OUT OF SCOPE
- Which v0 source type to use (git vs zip) — covered by existing CLI docs
- The shape of `docs/design-manifest.yaml` — covered by the v0_manifest module
- Patching compiler options or other non-`exclude` fields in `tsconfig.json`
- Retro-actively patching `tsconfig.json` in projects that never invoke `set-design-import` — out of scope by design
