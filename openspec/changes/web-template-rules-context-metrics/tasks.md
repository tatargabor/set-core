...
## Part A — wt-project-web template corrections
_Repo: `wt-project-web`_
...
- [x] `[REQ: nextjs-template-corrections / Rule is deployed by wt-project init]`
...
- [x] Added `_inject_feature_rules(project_path, wt_path, scope, spec_files)` in `lib/wt_orch/dispatcher.py` after `_build_pk_context()`
...
- [x] Added `CONTEXT_WINDOW_SIZE = 200_000` constant to `lib/wt_orch/verifier.py`
...
- [x] Added `context_tokens_start: Optional[int] = None` and `context_tokens_end: Optional[int] = None` to `Change` dataclass in `lib/wt_orch/state.py`
...
- [ ] Deploy updated wt-project-web templates to a test project: `wt-project init --project-type web` — `pnpm build` must succeed without manual config fixes
