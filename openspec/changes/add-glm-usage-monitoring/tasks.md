...
- [x] 1.1 Create `lib/set_orch/usage/glm.py`: `KIND_GLM`, `discover_glm_account(loader=None)`
...
- [x] 1.2 Write `tests/unit/test_usage_glm.py`: discovery (configured credential, no
...
- [x] 2.1 `lib/set_orch/usage/poller.py`: `UsageSource(discover, client)` dataclass;
...
- [x] 2.2 `lib/set_orch/usage/__init__.py`: export the new names; `default_sources()`
      returning the Claude pair plus the GLM pair
- [x] 2.3 `lib/set_orch/server.py`: construct the poller with `sources=default_sources()`
      — the only server change
- [x] 2.4 Extend `tests/unit/test_usage_poller.py`: two sources reach the snapshot; one
...
- [x] 2.5 Extend `tests/unit/test_usage_api.py`: a `kind: glm` account in the body still
      carries no credential

## 3. Frontend

- [x] 3.1 `web/src/lib/fleetUsageBars.ts`: window-label fallback deriving the span from
...
- [x] 3.2 `web/src/components/FleetUsageStrip.tsx`: reword the two critical-mark strings so
      they no longer attribute the band to the service ("over the critical threshold", not
      "windows the service calls critical")
- [x] 3.3 Extend `web/tests/unit/fleetUsageBars.test.ts`: GLM row beside Claude rows;
...
- [x] 3.4 Extend `web/tests/unit/fleetUsageStrip.test.tsx`: GLM bars beside Claude bars
...
- [x] 4.2 vitest: the three touched suites, including the two severity tripwires
- [x] 4.3 Visual check in the browser against the running dashboard: GLM bar pair beside
...
- [x] 4.4 Record the change in the living record if any consumer-facing behaviour note is
