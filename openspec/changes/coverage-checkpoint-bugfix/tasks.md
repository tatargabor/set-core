...
- [x] 1.1 In `bin/wt-sentinel` `check_orchestrator_liveness()`, read state file status and return 0 (not stuck) when `status == "checkpoint"`. On `jq` parse failure use `|| return 0` (fail safe — skip the kill, not proceed with it).
