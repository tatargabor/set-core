## IN SCOPE
- Automatic purge of orphan fix-iss children when `_check_affected_change_merged` auto-resolves an issue because the parent merged natively.
- Structured cleanup: remove the state.changes entry AND the openspec directory, with safe-remove guards.
- Idempotent re-escalation: if a parent already has a live `fix_iss_child` link, `escalate_change_to_fix_iss` returns the existing name instead of creating a duplicate.
- Operator CLI for retroactive orphan cleanup across all registered projects, dry-run supported.

## OUT OF SCOPE
- Cleanup of fix-iss children inside `openspec/changes/archive/` — historical records stay untouched.
- Cross-parent coalescing (one fix-iss child serving multiple parents).
- Changes to the `fix-iss-NNN-<slug>` naming convention.
- Migration of prior-run orphans — those are handled case-by-case via the CLI, not automated.
