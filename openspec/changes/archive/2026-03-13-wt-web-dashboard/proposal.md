...
- Create a FastAPI backend in `lib/wt_orch/` that exposes the existing Python core (state, process, templates) as REST + WebSocket endpoints
...
- Add `wt-orch-core serve` subcommand that starts the API server with static file serving
...
- `web-service-lifecycle`: systemd user service for always-on operation, `wt-orch-core serve` CLI entry point, install.sh integration for automatic deployment

### Modified Capabilities
- `orchestration-engine`: The `wt-orch-core` CLI gains a `serve` subcommand; `cli.py` extended with server bootstrap

## Impact

- **New files:** `lib/wt_orch/api.py`, `lib/wt_orch/watcher.py`, `lib/wt_orch/server.py`, `web/` (Vite React SPA), systemd service file
- **Modified:** `lib/wt_orch/cli.py` (add `serve` subcommand), `install.sh` (systemd deployment), `pyproject.toml` (fastapi, uvicorn, watchfiles deps)
