...
- **Approve via state file**: Same atomic write as `set-orchestrate approve` — writes `checkpoints[-1].approved = true` to orchestration-state.json
...
- **New files**: `bin/set-orchestrate-tui` (or inline in set-orchestrate as tui subcommand), Python Textual app
- **Modified files**: `bin/set-orchestrate` (add `tui` subcommand dispatch)
