...
- [x] 2.1 Update `orch_recall` in orch-memory.sh to exclude `stale:true` tagged memories (add exclusion logic or pass to set-memory)
...
- [x] 5.4 Add decompose skill and command to `set-project init` deploy list (auto — cp -r copies all wt/ skills)

## 6. Agent-Based Planning Path

- [x] 6.1 Add `plan_method` directive parsing to `parse_directives()` in planner.sh (default: `api`)
- [x] 6.2 Add `DEFAULT_PLAN_METHOD="api"` constant alongside other defaults in set-orchestrate
...
- [x] 6.7 Add planning-specific token budget constant (default 500K) in set-orchestrate
...
- [x] 8.3 Add test for `plan_memory_hygiene()` — verify it runs without error when set-memory is unavailable
