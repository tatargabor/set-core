...
- [x] 10.14 In `dispatcher.dispatch_ready_changes()`, consult `.set/issues/registry.json` and skip any pending change whose name matches an Issue currently in an active state (`investigating`/`diagnosed`/`awaiting_approval`/`fixing`/`verifying`/`deploying`/`failed`). The ISS pipeline runs `/opsx:ff` + `/opsx:apply` sequentially in the set-core directory; a parallel worktree dispatch on the same openspec change produces git conflicts. Helper `_iss_owned_change_names()` returns `set()` when the registry is absent so projects without ISS keep the engine-only dispatch path [REQ: auto-escalate-to-fix-iss-on-retry-budget-exhaustion]
