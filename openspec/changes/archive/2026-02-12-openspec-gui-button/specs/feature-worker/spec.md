...
The `_create_project_header` method SHALL read feature status from `self._feature_cache[project]` instead of calling subprocesses. The `get_memory_status` method SHALL read from cache and NOT run subprocess calls.
