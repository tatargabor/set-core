...
- **Cut over `merge_change()`**: Route bash `merge_change()` through `wt-orch-core merge execute` (Python). Fill gaps: `update_coverage_status()`, `_sync_running_worktrees()`, `fix_base_build_with_llm()`, hook dispatch.
...
- **Code**: `lib/orchestration/monitor.sh`, `merger.sh`, `planner.sh`, `digest.sh` become thin wrappers or are deleted. `lib/wt_orch/engine.py`, `merger.py`, `planner.py`, `digest.py`, `cli.py` become the live implementations.
- **Entry points**: `wt-orchestrate start` behavior unchanged externally but internally routes through Python.
