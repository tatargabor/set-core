...
- [x] 1.1 Add `send_summary_email()` to `lib/set_orch/notifications.py` — HTML email with state summary + coverage results via Resend API
- [x] 1.2 Add `trigger_checkpoint()` to `lib/set_orch/engine.py` — set state status to "checkpoint", log reason, emit CHECKPOINT event
- [x] 1.3 Add `final_coverage_check()` to `lib/set_orch/digest.py` — read digest requirements JSON, compute covered/uncovered counts, return summary string
- [x] 1.4 Add `update_coverage_status()` to `lib/set_orch/digest.py` — update requirement coverage after merge (mark mapped requirements as covered)
- [x] 1.5 Add `orch_memory_audit()` to `lib/set_orch/orch_memory.py` — periodic memory health check (dedup dry-run + spot-check)
...
- [x] 2.1 Add `cleanup_orchestrator()` function to `lib/set_orch/engine.py` — update state to "stopped" (unless "done"), kill dev server PIDs, pause running changes if directive set
...
- [x] 2.4 Add `set-orch-core engine monitor` CLI subcommand to `lib/set_orch/cli.py` — accept `--directives`, `--state`, `--poll-interval` args, call `engine.monitor_loop()`

## 3. Feature Flag & cmd_start() Cutover

- [x] 3.1 Add `ORCH_ENGINE` env var check to `cmd_start()` in `lib/orchestration/dispatcher.sh` — if "python" (or default after cutover), exec to `set-orch-core engine monitor` instead of calling bash `monitor_loop()`
...
- [x] 4.1 Add hook execution to `lib/set_orch/merger.py:merge_change()` — call pre_merge and post_merge hooks via `subprocess_utils.run_command()`
- [x] 4.2 Add `_sync_running_worktrees()` to `lib/set_orch/merger.py` — after merge, iterate running changes and call `dispatcher.sync_worktree_with_main()` for each
...
- [x] 5.1 Implement `auto_replan_cycle()` fully in `lib/set_orch/engine.py` — collect replan context, archive completed changes, call Claude for new plan, validate, dispatch
...
- [x] 6.1 Add `run_planning_pipeline()` to `lib/set_orch/planner.py` — orchestrate full flow: input detection, freshness check, triage gate, design bridge, Claude call, response parse, plan enrichment
...
- [x] 6.4 Add `populate_coverage()` to `lib/set_orch/digest.py` — map plan changes to digest requirements
- [x] 6.5 Add `check_coverage_gaps()` to `lib/set_orch/digest.py` — warn about uncovered requirements after plan creation
...
- [x] 7.5 Update `lib/orchestration/monitor.sh` header comment — "monitor_loop runs in Python via set-orch-core engine monitor"
