<!-- The scope blocks sit BEFORE the delta header, not inside it. Measured
     2026-08-20 on this very file: the delta parser ends a section at any `##`
     heading, so `## IN SCOPE` written after `## ADDED Requirements` — which is
     what the generated instruction asks for — truncates the section to zero
     requirements. The change then fails as "no deltas found", which is the
     lucky direction; the dangerous one is a change that validates while
     carrying nothing. -->
