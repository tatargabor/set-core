...
- **Per-lineage plan + digest retention.** Switching lineages preserves the prior lineage's `orchestration-plan.json`, `orchestration-plan-domains.json`, and the entire `set/orchestration/digest/` decomposition tree as renamed sibling copies (`orchestration-plan-<slug>.json`, `digest-<slug>/`). Readers pick the lineage-specific copy when the requested lineage is not the live one. Without this, switching the sidebar to v1 would show v2's digest — wrong data, wrong decisions.
