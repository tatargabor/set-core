...
- [x] 1.1 In `bin/set-sentinel` L455, change `"$SCRIPT_DIR/set-orchestrate" start "$@" &` to `"$SCRIPT_DIR/set-orchestrate" start "$@" 9>&- &`

## 2. Reinstall deps after worktree sync (Bug #29/#33)

- [x] 2.1 In `lib/set_orch/dispatcher.py`, add helper function `_reinstall_deps_if_needed(wt_path: str, old_sha: str, new_sha: str) -> None` that: (a) diffs old_sha..new_sha for lockfile/package.json changes, (b) if found, runs PM install via `_detect_package_manager`, (c) logs result, non-blocking on failure
