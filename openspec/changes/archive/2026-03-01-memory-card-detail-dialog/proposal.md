...
Memory cards in the Browse dialog truncate content to 200 characters. There's no way to see the full text of a memory without using the CLI (`set-memory get <id>`). Users need to click a card to read the complete content.
...
- The detail dialog fetches full content via `set-memory get <id>` to ensure nothing is truncated
