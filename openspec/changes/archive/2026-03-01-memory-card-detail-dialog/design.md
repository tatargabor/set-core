...
The `MemoryBrowseDialog` shows memory cards with content truncated to 200 characters. The `set-memory get <id>` command returns the full memory record. There's currently no way to view full content in the GUI.
...
**3. Fetch full content via `set-memory get <id>`**
Even though the card's `mem` dict may already contain the full content (in list/search mode), always call `set-memory get` to guarantee completeness and consistency. The call takes <100ms.
