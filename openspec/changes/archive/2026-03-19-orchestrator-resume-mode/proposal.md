...
2. **`set-orchestrate start` always re-plans**: if plan file is missing (because sentinel deleted it), triggers full digest + decompose (~5-10 min)
3. **No lightweight resume**: the only way to restart the monitor is through `cmd_start` which runs the full init pipeline

Real impact: after deploying a set-core fix mid-run, restarting takes 5-10 min (unnecessary decompose) or loses state entirely (sentinel fresh start).

## Solution

Add `set-orchestrate start --resume` (or detect automatically) that:
...
### In Scope
- `--resume` flag on `set-orchestrate start` (skip planning, use existing state)
...
- `set-orchestrate resume` alias (same as `start --resume`)
