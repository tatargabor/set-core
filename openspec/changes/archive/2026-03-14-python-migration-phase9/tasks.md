...
- [x] Create `lib/set_orch/loop_state.py` with `LoopState` dataclass and `init_loop_state()`, `read_loop_state()`, `update_loop_state()` (atomic with flock)
...
- [x] Create `lib/set_orch/loop_tasks.py` with `find_tasks_file()` — search worktree for tasks.md (root → subdirs)
...
- [x] Create `lib/set_orch/loop_prompt.py` with `detect_next_change_action()` — return ff/apply/done/none
...
- [x] Create `lib/set_orch/loop.py` with `classify_api_error()` — scan log for 429, rate-limit, 5xx patterns
...
- [x] Register CLI subcommands: `set-orch-core loop run|status`
- [x] Create `tests/unit/test_loop.py` — API error classification, backoff, completion, token budget tests

### 5. Hook Utilities Module

- [x] Create `lib/set_hooks/__init__.py` and `lib/set_hooks/util.py` with `_dbg()`, `_metrics_timer_start/end()`, `read_cache()`, `write_cache()` (atomic)
- [x] Create `lib/set_hooks/session.py` with `dedup_clear()`, `dedup_check()`, `dedup_add()`, `content_hash()`
...
- [x] Create `lib/set_hooks/memory_ops.py` with `recall_memories()` — call `set-memory recall`, parse JSON, dedup filter
- [x] Add `proactive_context()` — call `set-memory proactive`, return scored memories
...
- [x] Create `lib/set_hooks/stop.py` with `flush_metrics()` — collect session metrics, call `lib.metrics.flush_session()`
...
- [x] Create `lib/set_hooks/events.py` with `handle_event()` dispatcher — route by event type
...
- [x] Update `bin/wt-loop` — bin/wt-loop stays bash (CLI/terminal mgmt), loop utilities exposed via `set-orch-core loop` subcommands
- [x] Update `bin/wt-hook-memory` — replace `source lib/hooks/*.sh` with Python entry point `python3 -m set_hooks`
