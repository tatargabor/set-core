...
- State persistence across shutdown: `shutdown_at` timestamp, per-change `last_commit` hash
- Resume detection on sentinel startup: detect incomplete shutdown, validate worktrees, resume
- E2E scaffold `--project-dir` flag for persistent (non-volatile) project directories
- wt-web Settings page: Shutdown button with confirmation, status indicator, Resume action
- API endpoint `POST /api/{project}/shutdown` that triggers graceful shutdown
