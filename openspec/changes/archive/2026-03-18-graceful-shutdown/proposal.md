...
- **New**: `set-sentinel --shutdown` command that performs graceful shutdown: signals agents to finish current task, waits for clean exit, saves resume metadata, and stops the sentinel
...
- **New**: API endpoint `POST /api/{project}/shutdown` that triggers `set-sentinel --shutdown`
...
- **Sentinel**: `bin/set-sentinel` — new `--shutdown` flag, modified signal handlers, resume detection on startup
...
- **Engine**: `lib/set_orch/engine.py` — resume validation (verify worktrees exist, branches match)
