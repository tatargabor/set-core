...
The orchestrator SHALL provide a `set-orchestrate skip <name>` subcommand that transitions a change to "skipped" status.

#### Scenario: Skip a failed change
- **WHEN** `set-orchestrate skip my-change` is invoked
...
- **WHEN** `set-orchestrate skip my-change --reason "spec issues, will revisit"` is invoked
...
- **WHEN** `set-orchestrate skip nonexistent` is invoked
