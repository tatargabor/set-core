...
#### Scenario: Snapshot file created from local shadcn parse
- **WHEN** no design MCP is available but `detect_shadcn_project()` returns 0
- **AND** the shadcn parser generates a valid snapshot
- **THEN** the snapshot is written to the same `$DESIGN_SNAPSHOT_DIR/design-snapshot.md` path
- **AND** the metadata header indicates `source: shadcn-ui, type: local`
- **AND** subsequent pipeline stages (dispatch, verify) consume it identically to MCP-fetched snapshots
...
- **AND** `force` is `False`
- **THEN** the Python planner reads the cached file without invoking bash bridge or shadcn parser
...
- **THEN** `fetch_design_snapshot "force"` is called via bash subprocess (for MCP sources) OR the shadcn parser is re-invoked (for local sources)
