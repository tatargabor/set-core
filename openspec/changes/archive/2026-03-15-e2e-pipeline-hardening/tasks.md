...
- [x] 1.1 Create `git_has_uncommitted_work(wt_path) -> tuple[bool, str]` in `lib/wt_orch/git_utils.py` (new file, shared by loop_tasks and verifier) — runs `git status --porcelain` with 10s timeout, parses output into counts (modified, untracked), returns `(has_work, summary)`. Fail-open on timeout/error.
...
- [x] 2.1 Add uncommitted-check step to `handle_change_done()` in `lib/wt_orch/verifier.py` — insert before VG-BUILD step (after merge-rebase fast path early return). Import `git_has_uncommitted_work` from `git_utils`. If returns True, set `verify_ok = False` with reason including summary.
...
- [x] 3.1 Create `generate_startup_guide(wt_path) -> str` function in `lib/wt_orch/dispatcher.py` — detect PM from lockfile, framework dev command from package.json scripts, DB tool (Prisma/Drizzle) from dependencies, Playwright config existence. Return markdown section content.
...
- [x] 4.1 Add `auto_detect_smoke_command(directory) -> str` to `lib/wt_orch/config.py` — resolution chain: explicit config → build+test → test_command fallback. Detect build script from package.json, combine with PM.
...
## 5. Template Updates (wt-project-web)
