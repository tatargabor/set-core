...
### New Capabilities
- `nextjs-template-corrections`: Correct template files and rules in `set-project-web` to eliminate per-run rediscovery of known Next.js/Prisma/Tailwind configuration issues
...
- **set-project-web** (`wt_project_web/templates/nextjs/`): postcss.config.mjs, jest.config.ts, next.config.js, rules/data-model.md, rules/testing-conventions.md, new rules/worktree-setup.md
- **set-core dispatcher** (`lib/set_orch/dispatcher.py`): reads project-knowledge.yaml, resolves feature rules, copies to worktree `.claude/rules/`
- **set-core Ralph loop** (`lib/loop/state.sh` or set_orch layer): reads context utilization at loop start/end via Claude API usage headers or wt-usage
