...
- **Fix surrogate propagation in `_stop_raw_filter()`**: Sanitize lone surrogates from Node.js JSONL transcripts (which can contain unpaired surrogate escapes) before passing content to `set-memory remember`. Currently the `except Exception: pass` on line 1078 silently drops entries with surrogates.
- **Add UTF-8 sanitization in `set-memory cmd_remember`**: Defense-in-depth — sanitize content in the Python layer before passing to the Rust/PyO3 `m.remember()` call, so no surrogate can ever reach RocksDB.
...
### New Capabilities
- `utf8-safe-content-handling`: Ensure all text flowing through the hook pipeline and into set-memory is valid UTF-8, with character-safe truncation and surrogate sanitization at every boundary.
...
- `stop-hook-extraction`: Fix the commit extraction path (lines 1139, 1238, 1261) to sanitize content before `set-memory remember`.
...
- **`bin/set-memory`**: Defense-in-depth sanitization in `cmd_remember` Python inline script (line ~438).
- **CLAUDE.md template** (used by `set-project init`): Stronger memory citation instructions.
