...
- Add `set-sentinel` to install.sh scripts list (was missing, causing `command not found`)
...
- `install.sh` — add set-sentinel to scripts list
- `bin/set-orchestrate` — checkpoint auto-approve option
