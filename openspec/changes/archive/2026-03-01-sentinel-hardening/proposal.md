...
The current `set-sentinel` is a dumb bash loop — it restarts on crash with backoff but cannot reason about *why* something failed. It also has bugs: restarts on `time_limit` (ignoring user intent), doesn't handle all states, has no logging or notifications.
...
- Replace `bin/set-sentinel` bash loop with an agent skill (`/wt:sentinel`)
- Agent starts `set-orchestrate start` in background, then polls state.json + logs
...
- `bin/set-sentinel` — replaced by agent skill (kept as minimal fallback or removed)
