...
- [x] 1.2 Implement brief parser function in `bin/set-orchestrate`: extract Next items (bullet list), parse directives (key: value format), compute SHA-256 hash, apply defaults for missing directives
...
- [x] 2.1 Create `bin/set-orchestrate` with subcommand dispatch: plan, start, status, pause, resume, replan, approve — source wt-common.sh for shared functions
...
- [x] 2.4 Implement `set-orchestrate status` subcommand: read state file, display table with per-change name/status/progress/tokens, show merge queue, show brief staleness indicator
...
- [x] 3.1 Implement `set-orchestrate plan` subcommand: read brief, collect existing spec names from `openspec/specs/`, collect active changes from `openspec/changes/`, query `set-memory recall` for top 5 relevant memories
...
- [x] 3.4 Implement `set-orchestrate plan --show`: display plan in human-readable format with ASCII dependency graph (topological sort)
...
- [x] 4.3 Implement `set-orchestrate start` subcommand: validate plan exists, initialize orchestration-state.json, dispatch all ready changes (no dependencies + under parallel limit)

## 5. Progress Monitor Loop

- [x] 5.1 Implement monitor loop in `set-orchestrate start`: poll each active worktree's `.claude/loop-state.json` every 30 seconds, update orchestration-state.json with current iteration/tokens/status
...
- [x] 6.3 Implement checkpoint merge policy: add done changes to merge queue array in state, execute queue only on `set-orchestrate approve --merge`
...
- [x] 7.1 Implement `set-orchestrate pause <change>`: read ralph-terminal.pid from worktree, send SIGTERM, update change status to "paused"
- [x] 7.2 Implement `set-orchestrate pause --all`: iterate all running changes, pause each, set orchestration status to "paused"
- [x] 7.3 Implement `set-orchestrate resume <change>`: cd to existing worktree, `wt-loop start --max 30 --done openspec`, update status to "running"
- [x] 7.4 Implement `set-orchestrate resume --all`: resume all paused changes respecting max_parallel
- [x] 7.5 Implement `set-orchestrate replan`: read current state + updated brief, call Claude with both (include done/active/pending info), write updated plan with incremented plan_version, preserve merged changes
...
- [x] 8.3 Implement desktop notifications: `notify-send "set-orchestrate" "<one-line summary>"` on checkpoint, `notify-send -u critical` on failure — skip when notification directive is "none"
...
- [x] 8.5 Implement `set-orchestrate approve` subcommand: write approved=true to latest checkpoint entry, resume orchestration loop
- [x] 8.6 Implement `set-orchestrate approve --merge`: approve + execute all queued merges before resuming
...
- [x] 10.1 Implement `set-orchestrate self-test` subcommand: run Level 1 unit tests internally — parse_brief with sample input, parse_directives defaults/validation, topological_sort with known graph, state JSON roundtrip, circular dependency detection
...
- [x] 10.3 Level 2 integration test script: `tests/orchestrator/test-plan.sh` — create temp project dir, copy sample brief, run `set-orchestrate plan`, validate orchestration-plan.json structure (change names kebab-case, deps valid, no circular deps)
- [x] 10.4 Level 3 end-to-end test script: `tests/orchestrator/test-e2e.sh` — create temp git repo with trivial brief ("add hello.txt"), run `set-orchestrate plan` + `start`, wait for completion, verify change went through full lifecycle (pending→dispatched→running→done→merged), verify hello.txt exists on main
