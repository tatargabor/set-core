## IN SCOPE
- What a project declares it wants, and the difference between declaring nothing and declaring an empty set
- What is recorded as installed in a project, and when that record is written
- Which files a module places in a project, and which it never places
- Comparing an expected version against an installed one, including the case where neither can be read
- Performing an install: writing the declared files, refusing a missing requirement, and reporting what was skipped
- The report's obligations in both directions — what was written, what was not, and a run that changed nothing

## OUT OF SCOPE
- Any screen or route that offers an install (`fleet-view` 6.5, 7.15, 9.17 consume this capability)
- Deploying a project type's own templates (`project-init-deploy`), whose per-file writer this reuses
- Installing a module's executable part — a module's code is installed by a package manager, never copied
- Uninstalling or downgrading a module
- Choosing which module a capability belongs to
