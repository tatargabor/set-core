...
The hook-driven memory system (shodh-memory + wt-deploy-hooks) has replaced the deprecated `set-memory-hooks install` workflow. SYN-06 benchmark confirms +34% quality, -20% tokens. However there is no migration guide for existing users, no CHANGELOG documenting breaking changes, the README "Latest update" date is stale (2026-02-17), and deprecated specs/tools need a clear sunset plan. Before tagging a release, we need documentation consistency and a migration path.

## What Changes

- **BREAKING**: Document that `set-memory-hooks install` is fully deprecated — users must run `wt-deploy-hooks` instead
...
- Audit skill SKILL.md files for stale memory references (manual `set-memory recall` instructions that are now handled by hooks)
- Clean up deprecated OpenSpec specs: mark `memory-hooks-cli` and `memory-hooks-gui` with sunset timeline
- Verify CLI `--help` text consistency across `set-memory`, `wt-deploy-hooks`, `set-project`

## Capabilities

### New Capabilities
- `release-migration-guide`: Migration documentation for users upgrading from legacy set-memory-hooks to hook-driven memory
...
- `bin/set-memory --help`, `bin/wt-deploy-hooks --help`, `bin/set-project --help` (consistency check)
