...
- E2E runner script (`tests/e2e/run.sh`) ami a scaffold-ot friss git repo-ba klónozza, `set-project init --project-type web`-et futtat, és felkészíti a sentinel futtatásra
- Git tag-alapú checkpoint rendszer a runner-ben — minden fázis után tag, hogy javítás után visszatérhessünk
- A teszt a valódi set-core pipeline-t használja (sentinel → orchestrate → wt-loop), nincs mock — a valódi bugok valódian jönnek elő
...
- `e2e-runner`: Runner script és checkpoint infrastruktúra — scaffold klónozás, set-project init, git tag-ek, checkpoint-ból folytatás
...
- Nem érinti a meglévő set-core kódot — tisztán additív
