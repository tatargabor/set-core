## IN SCOPE
- A durable, per-project record of every agent discovery has seen: session id, label, cwd, kind, first-seen, last-seen.
- The record is written from discovery's own answer, while the agents are live.
- Reading a project's record back after the processes are gone, including after a reboot.
- Stating, per entry, whether a resumable transcript exists for it right now.
- Removing an entry the user no longer wants, and bounding how long an unseen entry is kept.

## OUT OF SCOPE
- Restoring anything. Bringing the recorded list back is `agent-fleet-restore`.
- Transcript CONTENT of any kind — the record carries identity and timestamps only.
- Replacing discovery. The live list still comes from process state; this records it.
- Cross-machine sync. The record is local to the machine that made it.
- Recording one-shot (`-p` / `--print`) subprocesses, which are not sessions anyone is sitting at.
