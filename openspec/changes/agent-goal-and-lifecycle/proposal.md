...
- **Remaining context is already measured, per model, by the harness.** It hands a hook
  `context_window: {context_window_size, used_percentage, remaining_percentage, …}` on every render.
  Nothing has to ask the agent how full it is, and no constant is needed.
...
- `agent-goal-record`: an agent started by the framework carries a goal declared at the moment of the
  act — its text, its kind, who asked for it, and when — readable while the agent runs; an agent the
  framework did not start has no goal and is reported as having none rather than as having a trivial
  one.
...
- `context-window-metrics`: its **Requirement: Context window size constant** mandates
  ...
  `context_window_size` per model in the payload it hands a hook, and this session's own window is
  1M — so the constant is not merely inflexible, it is currently wrong, and every utilisation
  percentage computed from it is wrong by the same factor. The requirement becomes: the window size
...
- `lib/set_orch/fleet/` — the goal record is written by the component that starts an agent, because
  that is the only moment the relation exists. Layer 1 stays abstract: it knows about a goal, a kind
