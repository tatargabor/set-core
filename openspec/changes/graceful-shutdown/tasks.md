...
- [x] 7.1 Unit test: `--shutdown` with no running sentinel exits cleanly [REQ: sentinel-graceful-shutdown-command]
- [x] 7.2 Unit test: wt-loop SIGTERM during idle exits immediately [REQ: agent-graceful-stop-on-sigterm]
- [x] 7.3 Unit test: wt-loop SIGTERM during work commits WIP and exits [REQ: agent-graceful-stop-on-sigterm]
- [x] 7.4 Unit test: resume from `"shutdown"` state with valid worktrees [REQ: resume-after-shutdown]
- [x] 7.5 Unit test: resume with missing worktree resets to pending [REQ: resume-after-shutdown]
- [x] 7.6 Unit test: `POST /api/{project}/shutdown` returns ok when sentinel running [REQ: shutdown-api-endpoint]
...
- [x] AC-6: WHEN graceful shutdown occurs THEN state has `"shutdown"` status, `shutdown_at` timestamp, per-change `last_commit` [REQ: state-persistence-for-resume, scenario: state-updated-during-graceful-shutdown]
- [x] AC-7: WHEN sentinel starts with `"shutdown"` state THEN validates worktrees, resumes valid changes, resets invalid to pending [REQ: resume-after-shutdown, scenario: sentinel-starts-with-shutdown-state]
- [x] AC-8: WHEN resume finds missing worktree THEN resets change to pending, logs warning [REQ: resume-after-shutdown, scenario: resume-with-missing-worktrees]
- [x] AC-9: WHEN `run.sh --project-dir ~/path` THEN project created at `~/path/minishop-runN` [REQ: persistent-project-directory-for-e2e, scenario: e2e-scaffold-with-custom-directory]
- [x] AC-10: WHEN `run.sh` without `--project-dir` THEN uses `/tmp/` default [REQ: persistent-project-directory-for-e2e, scenario: e2e-scaffold-without-flag]
- [x] AC-11: WHEN client sends `POST /api/{project}/shutdown` THEN sentinel receives SIGUSR1, returns `{"ok": true}` [REQ: shutdown-api-endpoint, scenario: shutdown-via-api]
- [x] AC-12: WHEN user clicks Shutdown in Settings THEN confirmation dialog appears, on confirm calls API, shows spinner [REQ: settings-page-shutdown-controls, scenario: user-clicks-shutdown-in-settings]
- [x] AC-13: WHEN orchestration status is `"shutdown"` THEN Settings shows status badge and Resume button [REQ: settings-page-shutdown-controls, scenario: settings-page-shows-shutdown-state]
