## IN SCOPE
- Handing ONE path, named by a person's activation, to the desktop's default application.
- The refusals that keep this from becoming a way to execute something.
- Saying which machine's desktop the file opens on.
- The answer shape: opened, or refused with a reason the reader can act on.

## OUT OF SCOPE
- Reading, writing or listing file content — that is `project-file-access`, and this
  capability deliberately does not become a second route to it.
- Answering whether a path exists, other than as the outcome of an activation somebody made.
- Choosing WHICH application opens the file. The desktop's own association decides; the
  framework does not maintain a handler table.
- What counts as a path in terminal output (`terminal-file-links`).
- Opening anything on the reader's browser machine when that is not the machine running the
  framework.
