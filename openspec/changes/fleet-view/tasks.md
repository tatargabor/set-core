...
- [~] 5.3 Relay terminal output and browser keystrokes in both directions, persisting nothing; diagnostics name the stream and failure kind only. **PARTIAL 2026-08-18 — the OWNER half only, and the name of this task is wider than what is delivered.** `ownerd.py` drains every held pty continuously into a bounded in-memory tail (mandatory, not optional — see 5.8's measurement), answers `tail`, and passes `write`/`resize` through; bytes travel base64 so a pty read that splits a UTF-8 sequence is not silently lost. Nothing is written to disk or to a log, and the diagnostics name the stream and the failure kind only. **Still open: nothing carries those bytes to a browser** — that is 6.4's stream endpoint and group 8, and neither exists. [REQ: terminal-traffic-travels-in-both-directions-and-is-never-persisted]
