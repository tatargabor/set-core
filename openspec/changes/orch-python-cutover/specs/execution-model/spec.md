...
The `wt-orchestrate start` command SHALL initialize orchestration state and launch the monitor loop. When `ORCH_ENGINE=python` (or after default cutover), `cmd_start()` SHALL exec to `wt-orch-core engine monitor` instead of calling bash `monitor_loop()`.
...
- **THEN** `cmd_start()` SHALL exec `wt-orch-core engine monitor --directives <path> --state <path> --poll-interval <int>`
