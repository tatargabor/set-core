...
- [x] 5.1 Create `web/src/lib/fleetUsageBars.ts` beside `fleetCacheHeat.ts`: one function turning an account record into what to draw — outcome, the two stripe fractions, severity tone, and the unmeasured mark [REQ: each-window-draws-consumption-against-elapsed-time]
- [x] 5.2 The elapsed stripe is computed in the browser from `resets_at` and the window length; consumption, severity and outcome are taken from the server unchanged [REQ: each-window-draws-consumption-against-elapsed-time]
- [x] 5.3 Unit-test the renderer: consumption ahead of elapsed, consumption behind elapsed, a null window, an unreachable account, an empty account list [REQ: an-unmeasured-window-is-marked-never-drawn-as-an-empty-bar]
- [x] 5.4 Unit-test that a critical severity produces the critical tone and a normal one never does, whatever the percentage [REQ: colour-states-the-upstream-severity-and-one-weight-means-one-thing]
- [x] 5.5 Render the strip in the fleet header of `web/src/pages/Fleet.tsx`, one row per account, both windows per row, with data attributes a test can aim at [REQ: the-strip-belongs-to-the-header-because-the-quota-is-not-a-per-agent-fact]
- [x] 5.6 Assert in a test that no consumption mark is rendered on any agent tab or tile [REQ: the-strip-belongs-to-the-header-because-the-quota-is-not-a-per-agent-fact]
- [x] 5.7 Show `measured_at` on the strip once the measurement is older than one polling interval, and keep the last good figures when a refresh fails [REQ: the-strip-says-how-old-its-measurement-is]
- [x] 5.8 Make the strip collapsible, and keep a critical window marked in the collapsed state [REQ: collapsing-the-strip-never-hides-a-critical-account]
- [x] 5.9 Test that the header and the screen below render fully when the usage request fails or never returns [REQ: the-header-renders-whether-or-not-the-measurement-arrived]
- [x] 5.10 Verify the critical colour is used for nothing decorative in the strip [REQ: colour-states-the-upstream-severity-and-one-weight-means-one-thing]

## 6. Verification on the running system

- [x] 6.1 Against the running server, read `/api/usage/accounts` twice inside one interval and show from the logs that no upstream call was made [REQ: the-measurement-is-polled-and-every-answer-says-when-it-was-taken] **DONE — 8 reads, 0 new upstream calls (`set_orch.usage.client: account` stayed at 6 in the journal), and all 8 answers carried one `measured_at`: `2026-08-27T19:25:53.623412+00:00`.**
- [x] 6.2 `pnpm build` in `web/`, so port 7400 serves the built strip [REQ: the-strip-belongs-to-the-header-because-the-quota-is-not-a-per-agent-fact]
- [x] 6.3 **LOOK AT IT (required, not optional):** open the fleet screen in Chrome and report what is actually on screen — every account row, both windows, the two stripes, and whether the header is crowded. If the browser cannot be reached, this task stays OPEN and the commit says so [REQ: the-strip-belongs-to-the-header-because-the-quota-is-not-a-per-agent-fact] **DONE — looked at in Chrome, 1512×790, against the running dashboard. It found TWO defects every green suite had missed, both now fixed and held by tests: (a) the two stripes were 3 px inside a 9 px box and read as ONE bar — the pair IS the measurement, so that is the single-bar reading this strip exists to replace; now 4 px + 1 px gap. (b) three expired credentials drew three IDENTICAL sentences, three rows deep — one cause stated three times, the same defect the header's owner chip already carries a note about; now one line naming the count, with the names on its tooltip. After the fix the strip is 97 px with three account rows and one shared line; the first account's 7-day bar is red at 97 % with its elapsed stripe at 89.6 % visibly shorter, which is the comparison the pair exists for.**
- [x] 6.4 **LOOK AT the unmeasured state**: with an account whose windows are null, confirm on screen that it shows the unmeasured mark and not an empty bar [REQ: an-unmeasured-window-is-marked-never-drawn-as-an-empty-bar] **DONE — one of the three live accounts is in exactly this state. On screen its row reads `5h ? 7d ? answered, and carried no figures` in amber, and the DOM carries no bar element for it at all (`data-fleet-usage-window="unmeasured"`, no `span.relative`). Beside it the two measured accounts draw bars, so the difference is visible without hovering.**
- [x] 6.5 **LOOK AT the collapsed state** with a critical account present, and report whether the critical mark survives the collapse [REQ: collapsing-the-strip-never-hides-a-critical-account] **DONE — collapsed, the strip is 25 px and shows `> ⌚6 ⚠1`: zero account rows, and the red critical mark still there. Re-opening restores all three rows.**
