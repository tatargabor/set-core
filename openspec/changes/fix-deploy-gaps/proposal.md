...
- **Add "Auto-Commit After Apply" section to CLAUDE.md template** in `deploy_wt_tools()` so all deployed projects get the commit instruction
...
- `bin/wt-project` — deploy_wt_tools() CLAUDE.md snippet addition
...
- All registered projects get the fix on next `install.sh` or `wt-project init` run
