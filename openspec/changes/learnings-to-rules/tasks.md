...
- [ ] 1.1 Create `lib/set_orch/learnings_analyzer.py` with `analyze_findings(project_path: str) -> list[RuleCandidate]` function: reads `set/orchestration/review-findings.jsonl`, extracts recurring patterns (same normalization as `generate_review_findings_summary()`: strip severity tag, first 50 chars, count across changes), filters by threshold (≥3 occurrences, ≥2 changes) [REQ: extract-recurring-patterns-from-findings]
...
- [ ] 1.7 Add `save_candidates(project_path: str, candidates: list[RuleCandidate])` — writes to `set/orchestration/rule-candidates.json` as JSON array. Add `load_candidates(project_path: str) -> list[RuleCandidate]` — reads and returns existing candidates [REQ: generate-rule-candidates-from-patterns]
