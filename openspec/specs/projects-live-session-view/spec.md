# Projects Live Session View

## Purpose

Answer "where is work happening right now" on the two surfaces that list projects — the projects
screen and the fleet's project column — without letting the answer hide anything. Liveness is
counted from the fleet's measurement of live agent processes, never inferred from a project's
own orchestration status, which has been measured reporting a project stopped while agents were
working inside it.
