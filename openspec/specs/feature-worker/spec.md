...
- **THEN** it iterates all known projects, runs `set-openspec status --json` (with cwd set to main repo), parses results, and emits `features_updated` with the combined dict
