...
- **WHEN** `set-status` checks a worktree on Linux
...
- **WHEN** `set-status` checks a worktree
...
- **WHEN** `set-status` checks a worktree on macOS
...
- **WHEN** `set-status` detects an agent with status "waiting" in a worktree
...
- **WHEN** `set-status` detects an agent in a worktree with no editor window
...
- **WHEN** `set-status` detects an agent with status "running" (session mtime < 10s) in a worktree
...
- **WHEN** `set-status` detects multiple waiting agents in a worktree with no editor
