...
## Purpose
**ARCHIVED** — The 5-layer hook system in `settings.json` (deployed by `wt-deploy-hooks` via `set-project init`) now handles all memory operations. The `set-memory-hooks install` command is deprecated. `check` and `remove` still work for cleaning up legacy inline hooks.
