...
## Purpose
**ARCHIVED** — The 5-layer hook system in `settings.json` (deployed by `set-deploy-hooks` via `set-project init`) now handles all memory operations. The `set-memory-hooks install` command is deprecated. `check` and `remove` still work for cleaning up legacy inline hooks.

## REMOVED Requirements

### Requirement: set-memory-hooks install command
**Reason**: DEPRECATED — The 5-layer hook system in `settings.json` (deployed by `set-deploy-hooks` via `set-project init`) now handles all memory operations. Inline SKILL.md patching is no longer needed.
...
**Migration**: Hooks are automatically deployed by `set-deploy-hooks`.

### Requirement: set-memory-hooks check command
**Reason**: DEPRECATED — Hook status is managed by `set-deploy-hooks` and `set-project init`.
