...
- **THEN** `set-status --json` returns an `agents` array with one entry containing `pid`, `status`, and `skill`

#### Scenario: Multiple agents on worktree
- **WHEN** two or more Claude processes have their CWD in the same worktree directory
- **THEN** `set-status --json` returns an `agents` array with one entry per process, each with its own `pid`, `status`, and `skill`

#### Scenario: No agents on worktree
- **WHEN** no Claude process has its CWD in a worktree directory
- **THEN** `set-status --json` returns an empty `agents` array `[]`
...
- **WHEN** `set-status` reads skill information for a worktree
...
- **WHEN** `set-status` reads `.set-core/agents/` directory
- **THEN** for each `.skill` file, it verifies the PID is alive using `kill -0 <pid>`
- **AND** removes files for dead PIDs

#### Scenario: Backward compatibility of set-skill-start
- **WHEN** `set-skill-start` writes a new per-PID skill file
...
The `set-status --json` output SHALL use an `agents` array instead of a single `agent` object.

#### Scenario: JSON format with agents array
- **WHEN** `set-status --json` is called
...
- **WHEN** `set-status --json` produces the summary object
