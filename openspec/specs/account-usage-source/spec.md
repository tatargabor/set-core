# Account Usage Source Specification

## Purpose

Owns the headless measurement of an account's rolling usage windows: which accounts this
machine holds credentials for, how each authenticates, what the upstream answer means, and
how *unreachable*, *unmeasured* and *measured at zero* are kept apart all the way to the
caller. It is free of any GUI import, so one measurement serves the web dashboard, the
desktop Control Center and any CLI.
