...
## Purpose
Provide CLI access to Claude token usage data via the `set-usage` command, with time range selection, cost estimation, and session key management.

## Requirements

### Requirement: CLI time range selection
The `set-usage` command SHALL support time range flags to query usage over different periods.

#### Scenario: Default 5-hour window
- **WHEN** user runs `set-usage` with no time flags
- **THEN** the output shows token usage for the last 5 hours

#### Scenario: Today's usage
- **WHEN** user runs `set-usage --today`
- **THEN** the output shows token usage since midnight UTC of the current day

#### Scenario: Weekly usage
- **WHEN** user runs `set-usage --week`
- **THEN** the output shows token usage for the last 7 days

#### Scenario: Monthly usage
- **WHEN** user runs `set-usage --month`
- **THEN** the output shows token usage for the last 30 days

### Requirement: CLI cost estimation
The `set-usage` command SHALL display estimated USD cost when requested.

#### Scenario: Cost flag shows estimated cost
- **WHEN** user runs `set-usage --cost`
...
- **WHEN** user runs `set-usage --today --cost`
- **THEN** the output shows today's token usage with estimated cost

### Requirement: CLI session key login
The `set-usage` command SHALL provide a `--login` flag for session key acquisition.

#### Scenario: Login flow opens browser
- **WHEN** user runs `set-usage --login`
...
- **WHEN** user runs `set-usage --login` on macOS
- **THEN** the browser is opened via `open` command

#### Scenario: Login flow on Linux
- **WHEN** user runs `set-usage --login` on Linux
...
- **WHEN** user runs `set-usage` and a valid session key exists
...
- **WHEN** user runs `set-usage` and no session key exists
- **THEN** the output shows token counts only (no percentages)
- **AND** no error is displayed

### Requirement: CLI text output formatting
The `set-usage --format text` output SHALL be human-readable with aligned columns.

#### Scenario: Text format output
- **WHEN** user runs `set-usage --format text`
...
### Files
- `bin/set-usage` - CLI entry point
- `gui/usage_calculator.py` - Shared token calculation logic
