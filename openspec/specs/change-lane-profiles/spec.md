# Change Lane Profiles Specification

## Purpose

Owns the set of valid change types, the per-type gate profile each one resolves to, and the
condition under which a type whose entrance is cheaper may be declared at all. The framework
defines the shape; a project supplies the mapping from its own lane vocabulary and the evidence
that its exit obligation blocks.
