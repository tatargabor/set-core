...
- **WHEN** `check_pid(pid=1234, expected_cmdline_pattern="set-loop")` is called and PID 1234 is alive running `set-loop start ...`
- **THEN** the function returns `True`

#### Scenario: Recycled PID with different command
- **WHEN** `check_pid(pid=1234, expected_cmdline_pattern="set-loop")` is called and PID 1234 is alive but running `/usr/bin/python3 other-script`
- **THEN** the function returns `False`

#### Scenario: Dead PID
- **WHEN** `check_pid(pid=9999, expected_cmdline_pattern="set-loop")` is called and PID 9999 does not exist
- **THEN** the function returns `False`
...
- **WHEN** `safe_kill(pid=1234, expected_cmdline_pattern="set-loop", timeout=10)` is called and the process exits within 3 seconds of SIGTERM
...
The system SHALL detect orphaned orchestration processes (set-loop instances with no corresponding active change in state). `find_orphans(expected_pattern, known_pids)` SHALL scan running processes matching the pattern and return those whose PIDs are not in the known set.

#### Scenario: Orphaned set-loop found
- **WHEN** `find_orphans(expected_pattern="set-loop", known_pids=[1234, 5678])` is called and PID 9999 is running `set-loop start ... --change my-change`
- **THEN** the function returns an OrphanInfo with pid=9999, cmdline containing "set-loop", and the extracted change name

#### Scenario: All processes accounted for
- **WHEN** all running set-loop processes have PIDs in the `known_pids` set
...
- **WHEN** bash calls `set-orch-core process check-pid --pid 1234 --expect-cmd "set-loop"`
...
- **WHEN** bash calls `set-orch-core process safe-kill --pid 1234 --expect-cmd "set-loop" --timeout 10`
- **THEN** stdout contains `{"result": "terminated", "signal": "SIGTERM"}` and exit code is 0

#### Scenario: find-orphans returns JSON list
- **WHEN** bash calls `set-orch-core process find-orphans --expect-cmd "set-loop" --known-pids "1234,5678"`
