...
The `set-web-install` command SHALL install the set-web systemd user service, enable it, start it, and verify it is running.

#### Scenario: Fresh install
- **WHEN** user runs `set-web-install` and no service exists
- **THEN** the service template is copied to `~/.config/systemd/user/set-web.service`, daemon is reloaded, service is enabled and started, and a health check confirms `localhost:<port>` responds

#### Scenario: Service already installed
- **WHEN** user runs `set-web-install` and the service file already exists
...
- **WHEN** user runs `set-web-install --port 8000`
...
- **THEN** the installer configures `tailscale serve --https 443` pointing to the local set-web port and displays the resulting HTTPS URL
...
- **WHEN** user runs `set-web-install --tailscale`
...
The installer SHALL support removing the set-web service.

#### Scenario: Uninstall
- **WHEN** user runs `set-web-install --uninstall`
