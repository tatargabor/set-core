...
- **THEN** `set-loop start` SHALL be called with `--test-command "<directives.test_command>"`
...
- **THEN** `resume_change()` SHALL NOT pass `--test-command` to `set-loop`
...
When `--test-command` is provided to `set-loop start`, the command SHALL be persisted in `loop-state.json` under the `test_command` key so that `_check_test_done()` can read it on each iteration.

#### Scenario: Test command persisted in loop state
- **WHEN** `set-loop start "fix" --done test --test-command "pytest -x"`
