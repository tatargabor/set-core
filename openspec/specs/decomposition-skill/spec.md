# Decomposition Skill Specification
...
- **THEN** the skill prompt SHALL instruct the agent to:
  ...
  2. Read project type config (`set/plugins/project-type.yaml`) if present
  3. Read project knowledge (`set/knowledge/project-knowledge.yaml`) if present
...
- **WHEN** `set/plugins/project-type.yaml` exists
...
- **WHEN** `set/plugins/project-type.yaml` does not exist
