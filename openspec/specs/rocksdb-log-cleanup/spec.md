...
### Requirement: set-memory cleanup-logs subcommand
The `set-memory` CLI SHALL provide a `cleanup-logs` subcommand that removes RocksDB `LOG.old.*` files older than 24 hours from the project's memory storage directories (`memories/` and `memory_index/`).

#### Scenario: Old LOG.old files removed
- **WHEN** `set-memory cleanup-logs` is run
...
- **THEN** `set-memory cleanup-logs` SHALL run before `_stop_run_extraction_bg`
