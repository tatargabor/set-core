...
The watchdog escalation chain SHALL use `set-orch-core process safe-kill` for process termination at escalation levels 3+ (redispatch/fail), replacing manual signal sequences.

#### Scenario: Escalation level 3 — redispatch with safe kill
- **WHEN** the watchdog escalates to level 3 and calls `redispatch_change()`
- **THEN** the redispatch function uses `set-orch-core process safe-kill` which verifies identity before SIGTERM and before SIGKILL
