...
- **AND** all write operations remain in wt skill (set-new, set-close, set-loop start)
...
- **THEN** MCP reads `<wt_path>/.claude/loop-state.json` directly
- **AND** does NOT spawn `set-loop status` subprocess

#### Scenario: Projects from config file
- **WHEN** MCP needs project list
- **THEN** reads from `~/.config/set-core/projects.json`
- **AND** does NOT call `set-list` command
