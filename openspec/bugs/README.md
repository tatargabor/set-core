...
### B-155 — every project watcher watched its whole tree recursively, so a consumer build starved the dashboard's event loop
- **renumbered:** B-149 → B-155 in the 2026-09-24 merge with origin; the other machine had already published B-149. Commit messages written before the merge cite B-149.
...
- **CLOSED:** `awatch(..., recursive=False)` over the state dir, the log dir, the root and the project-local orchestration dir; `rust_timeout`/`yield_on_timeout` wake the loop every `DIR_RECHECK_MS` to pick up a dir that appears later (what the recursive root watch used to cover); that wake does `is_dir` checks only, never `_refresh_paths`, which would spawn git on the loop every 5 s per project (B-146). `tests/unit/test_watcher_not_recursive.py`: the depth test measured **50** watches on the old watcher for a 40-deep tree (fails) and ≤4 on the new one; the late-dir test holds the pick-up half.

### B-154 — a terminal path to a file created after the terminal opened resolves against the project root, and the panel says "no such file"
- **renumbered:** B-148 → B-154 in the 2026-09-24 merge with origin; the other machine had already published B-148. Commit messages written before the merge cite B-148.
- **state:** CLOSED (2026-09-24, see the commit that carries this entry) — the entry stays; evidence below.
...
### B-153 — a docked panel's X undocks instead of closing, so closing a pinned panel takes two clicks
- **renumbered:** B-147 → B-153 in the 2026-09-24 merge with origin; the other machine had already published B-147. Commit messages written before the merge cite B-147.
...
### B-146 — the dashboard's file watcher resolved the state-file basename per CHANGED PATH, two git subprocesses each, on the event loop — so one build output in a watched root froze `set-web` for minutes while the process looked healthy
- **state:** closed (`cbbd7363`), verified live 2026-09-12 13:07 — the fixed process (PID 67010) received three large batches within 25 s, **6 259, 4 856 and 6 268 paths**, and `/api/fleet/owner` answered `200` in **1.4–16 ms** on every one of the six curls after each (one outlier at 136 ms); the same batch size cost **90 s** of dead loop on the old code two hours earlier. Not one stalled minute in the log since the restart (12:41 → 13:07). Probe: a 40-minute tail of `set-web.err` curling the endpoint six times, 2 s apart, after each `changes detected` line with N ≥ 1 000.
- **reported:** 2026-09-12 by the user — "set-web megint befagyott"
- **measured:** PID 562 (launchd `com.set-core.web`, up since 09-09 22:24) was alive, `*:7400` listening, 16 % CPU, and every HTTP request timed out (`curl -m 8` → `000` on `/`, `/api/health`, `/api/projects`) while the usage poller kept logging once a minute — so a thread was fine and the loop was not. `sample 562 3`: 2 293 of 2 301 main-thread samples inside ONE asyncio task step, three nested constructors deep — `_PyEval → type_call → __init__ → type_call → __init__ → [select_poll 1 800 | type_call → __init__ → subprocess_fork_exec 274 + os_read 123]` — i.e. `LineagePaths.__init__ → SetRuntime.__init__ → resolve_project_name → subprocess.run`, with the fork's child caught by `ps -P 562` as `git rev-parse --git-common-dir`. The site: `lib/set_orch/watcher.py` `watch()`, `for change_type, change_path in changes:` rebuilt `LineagePaths(str(self.project_path)).state_file` per path. The trigger, from the server log (`watchfiles.main: N changes detected`, N ≥ 1000, today): `11:46:59 6177`, `11:48:27 6320`, `12:13:16 6209`, `12:14:39 6374`, `12:24:45 6264`, `12:26:14 12639` — every stalled minute in the log (log lines/min < 30, usage poller only) starts at one of these. The 12 639-path batch (a Next.js `.next/` build output inside a watched project root: 10 288 under `standalone/`, 2 085 under `server/`) held the loop 12:26:15 → 12:29:11 = **177 s**; the 6 264-path one 12:24:45 → 12:26:14 = **90 s**. `SetRuntime(path)` measured at **12 ms** per construction on this machine (8 registered project roots, 0.10 s total), so 12 639 × 12 ms = 152 s and 6 264 × 12 ms = 75 s — the stalls are the batch size times the per-path cost, plus fork overhead on an 80 MB, 27-thread process (`_malloc_fork_parent` visible in the sample). Stalled minutes by day: 09-10: 2, 09-11: 21, 09-12: 5 by 12:35.
- **fail direction:** silent and convincing — every liveness signal a supervisor checks (PID, port, a log still growing) stays green, so a blocked event loop reads as "slow", and a client that reconnects when the loop wakes (the fleet terminal did, at 12:29:12) hides the gap from the screen. And it self-clears, which is the worst property for a bug report: by the time anyone samples it, the loop is free again.
- **fixed when:** a batch of N unrelated paths through `ProjectWatcher.watch` resolves the project name at most ONCE, and the next ≥ 1 000-path batch on the live server does not move `/api/fleet/owner` latency out of the sub-second range. The first half is `tests/unit/test_watcher_change_loop.py`: 500 resolutions on the old code, 1 on the fix — proved red-then-green in that order rather than by stash. The second half is the pending probe (a 40-minute tail of the log that curls the endpoint six times after each large batch).
- **not fixed here, and still open — the two neighbours:** B-110 is WHY a build output reaches the watcher at all (the project ROOT is watched recursively); with it fixed this batch never arrives. And the 30 s lifecycle tick (`api/lifecycle.py:_tick`) runs `SetRuntime` per registered project synchronously on the same loop — bounded at 0.1 s per tick with 8 roots here, but the same shape. Neither is a defect measured today; both are the same class (subprocess on the event loop) and should go together, off-loop.
- **scope note:** the process restarted at 12:40:30 (PID 67010) with the fix in its tree. The old PID held the code it started with — a shipped commit is not a running system. Also seen and NOT fixed: `set-web.err` is 610 MB of DEBUG output (`~660 lines/min` at idle); not the cause, but the reason every measurement here took a minute.

### B-147 — the Control Center's dialogs never got their native level raise, so a modal message box hides UNDER the always-on-top window and the app reads as frozen
- **renumbered on merge (2026-09-21):** registered as B-144 in commit `a9f733c4`; origin already held B-144 for the dry-run defect when the two registers met, and the published number wins.
- **state:** open
...
### B-148 — the Chrome session scan finds zero accounts, and one profile fails to decrypt
- **renumbered on merge (2026-09-21):** registered as B-145 in commit `a9f733c4`; origin already held B-145 for the module-script load defect when the two registers met.
- **state:** open — reported alongside B-147, NOT fixed by it
...
### B-143 — discovery admits a harness tool child wearing the agent's exe+comm, and the roster seats it as an unnamed interactive agent
- **state:** CLOSED (2026-09-11, `e3647a5f`) — the entry stays; evidence below.
- **reported:** 2026-09-11 by the user, with a FleetView screenshot: the third docked panel rendered `unnamed · unknown cache · provider unrecorded` over `no input: this process has no session id, so no seat can be resolved`, and the user asked whether the panel is broken or it is "an agent without a terminal". Re-reported the same day with a second screenshot showing FIVE `unnamed ? ?` tabs in one project's tab strip — the one-tile-per-armed-Monitor direction this entry predicted — asking to hide them.
- **measured:** neither — the panel is truthful and the process is not an agent. `ps -p <pid>` showed a Monitor pipeline stage (`tail -n0 -F <consumer worktree log> | grep --line-buffered -E "<domain patterns>"`), no tty, no children, ancestry `claude → bash → bash → <pid>`, owned by a live named session busy in the same project. The process itself wears the borrowed identity: `/proc/<pid>/comm` = `claude` AND `/proc/<pid>/exe` = the agent binary, while `/proc/<pid>/cmdline` = the grep. So `discovery._live_agent_pids` (executable identity, `lib/set_orch/fleet/discovery.py:143`) matched it, `_kind_of` found none of `NON_INTERACTIVE_FLAGS` in a bare grep argv and defaulted it to `interactive`, and `roster.record` wrote `no-session:<project>/pid-<pid>` with `kind: "interactive"` into `~/.local/share/set-core/fleet-roster.json` — measured in the live file. Reproduced with current HEAD: `discover_agents()` returned 8 agents, 7 real sessions plus this one wearing tool argv; its parent line on the surface even resolves the true owner via `discovery.parent_seat` (`source: "ancestry"`).
- **fail direction:** a watcher reads as a chat. The docstring rejects argv-substring identity because shells wear words in paths; the missed case is argv and exe DISAGREEING, which is a child wearing a parent's identity, and no `-p`-class flag catches a bare `grep` stage. One wrong tile per armed Monitor per project, each with a dead input box offered into a pipe.
- **fixed when:** discovery treats a claude-identity process whose `argv[0]` is not the agent binary as NOT an agent (the argv/exe disagreement is the structural test, mirroring `_is_waiter_argv`), with a unit test holding the measured shape (comm=agent, exe=agent binary, argv[0]=a tool) — and the surface either drops such processes or shows them as watchers of their ancestry-resolved parent seat, never as unnamed chats.
- **CLOSED by `e3647a5f`:** `discovery._argv_belies_identity` refuses a candidate on the argv's positive testimony (`basename(argv[0]) != AGENT_COMM`), applied in `discover_agents` before the kind split; an unreadable argv excludes nothing, and the exclusion is NOT keyed on the missing record — a session at its start-up trust prompt has no record either. The roster self-heals: with the process no longer reported, `_retire_unseen_sessionless` drops the sessionless entry on the next full-sweep record. The new test fails on unfixed source (stash-and-rerun) and the two remaining failures in the touched suites are the registered B-141/B-126, failing identically with the fix stashed. The "show as a watcher of the parent seat" half was NOT built — hiding is what the user asked for; if a watcher view is wanted it is a new request, not this defect.

### B-144 — `set-project init --dry-run` is not dry: it registers the project and lets the external OpenSpec CLI write 25 files into the tree
- **state:** open
- **reported:** 2026-09-12 by this session, while diagnosing why a repo was missing from FleetView — the dry run was chosen precisely because the safety track advertises it as the honest preview
- **measured:** ran `set-project init --dry-run` in a previously unregistered sibling repo. Its own deploy honoured the flag (`Would deploy 20 command(s)`, `Would write version`, `Would register with manager at http://localhost:7400 (dry run — not sent)`), but two real writes happened anyway:
  1. **Registry.** `~/.config/set-core/projects.json` went from 47 to 48 entries, the new one carrying `addedAt: 2026-09-12T12:49:14Z`. The printed line `Project '<name>' registered` is past tense, not conditional — and `GET /api/fleet/agents` then returned 50 projects instead of 49, with the new one present at `sources: ["registry"]`.
  2. **The tree.** `git status --porcelain` in that repo went from clean to `?? .claude/` + `?? openspec/`. `find .claude openspec -type f` = 26 files, of which exactly one (`.claude/settings.json`, dated 2026-08-19) predates the run: **25 files written** — `.claude/commands/opsx/*.md` (12), `.claude/skills/openspec-*/SKILL.md` (12) and `openspec/config.yaml`. The dry-run output itself prints the external tool's real completion banner (`OpenSpec structure created`, `Setup complete for Claude Code`, `Initialized OpenSpec`), so the writes are the **external `openspec init` call**, which never learns about the flag — the framework's own guarded write paths all held.
- **fail direction:** the worst available for this particular command. `--dry-run` is the one step the deploy-safety rule tells every session to run FIRST against a consumer tree, and the guarantee it is trusted for ("proven not to write in preview", verified by a 2477-file sha256 snapshot) is true only of the paths the framework itself owns. The snapshot that established it was taken on a tree where OpenSpec was already initialised, so the one call that writes had nothing left to create and the measurement came back zero — a preview is destructive exactly and only on a tree that has never been initialised, which is every first look at a new project.
- **earlier sighting, folded in on merge (2026-09-21):** first measured 2026-08-31 on a second machine, while registering a freshly cloned consumer repo: `set-project init --dry-run` printed `Project '<name>' registered` and `~/.config/set-core/projects.json` gained the entry in the same second — file mtime `Aug 31 12:45:24`, entry `addedAt: 2026-08-31T10:45:24Z`, both equal to the dry-run's run time; a read of the same file minutes earlier had no such entry (full key list, not a truncated read). Only the registry write was seen then — that tree already had OpenSpec initialised, so the 25-file write below had nothing to create, which is the blind spot this entry names. That sighting was registered as **B-143** in commit `81106898` before the two machines' registers met; the number here is the published one.
- **also:** the guard reasoning generalises past this call — a preview that shells out to any tool inherits that tool's writes, and the flag stops at the process boundary unless something carries it.
- **fixed when:** `set-project init --dry-run` performs **zero** writes on a never-initialised tree — the OpenSpec init step is skipped (and printed as `Would initialize OpenSpec`) and the registry entry is not made (printed as `Would register project '<name>'`) — proven by a test that runs the dry run against a fresh git repo fixture and asserts (a) `git status --porcelain` is empty and no new untracked paths exist, (b) the registry file's byte content is unchanged. Stash-and-rerun must show the test red on the unfixed source.

### B-145 — the dashboard's module script is sometimes never requested on load in an automation-driven Chrome tab, leaving `#root` empty with no error
- **state:** open — not yet shown to be a product defect
- **reported:** 2026-09-21 by this session, while looking at the fleet screen for `fleet-restore-across-projects`
- **measured:** three fresh navigations to `http://localhost:7400/` in a Claude-in-Chrome tab (`document.visibilityState === "hidden"`) left `#root` with 0 children after 8 s; `document.readyState` stayed `interactive`; `performance.getEntriesByType('resource')` listed the CSS asset (200) but **no request for the `index-*.js` module at all**; the console held no error. A manual `await import('/assets/index-<hash>.js')` rendered the screen at once. One navigation in the same tab rendered normally.
- **fail direction:** a blank page with no error, so it reads as "the dashboard is down" rather than as a load that never started.
- **open question, and it decides whether this is ours:** whether a visible, human-driven tab ever shows it. Nothing measured so far rules out the browser deferring module loads in a hidden automation tab.
...
### B-149 — the web unit suite is flaky: 8 of 9 failures come and go on identical source, so any regression set-diff against it lies in both directions
- **state:** open
- **reported:** 2026-09-21 by this session, while taking a baseline for `fleet-structured-notices` — noticed only because the first set-diff claimed the change had *fixed* four unrelated fleet tests, which it cannot have done. A result that flatters the change is the one to distrust.
- **measured:** four runs of `npm run test:unit` (node 24; node 18 dies with `ERR_REQUIRE_ESM`) over **identical** source — one before any edit, three after — gave **7, 3, 5 and 5** failures. Union across the four runs: **9 distinct tests**. Failing in *every* run: **1** (`tests/unit/designDrift.test.ts :: design-system drift carries no arbitrary font size — the scale is 12/14/16`). So 8 of 9 are non-deterministic. Two consecutive runs of the same code differed on four tests: `fleetArrangement.test.tsx :: renders a waiting agent as waiting, not as quiet`, `fleetSurface.test.tsx :: drops the phase where the header is already shouting it`, and two of `fleetSurface.test.tsx :: task 7.4 …`. Sets captured at `scratchpad/baseline/web-unit-flaky-union.txt` and `…/web-unit-stable-fails.txt`.
- **fail direction:** both, which is what makes it expensive. A change is credited with fixes it did not make, and — the costly half — a real regression it *did* cause sits inside the flaky set and reads as "pre-existing debt, ignore it". The repository's own regression discipline is a set diff against a baseline, and a flaky baseline silently degrades every such check made after it, in every change, without anything reporting that it has.
- **note:** this is the same class the project rules already name — *a debt figure is a measurement with a timestamp* — one level worse, because here the figure is not even stable within one commit. The count cannot be quoted at all; only a union across several runs is usable, and then only to say a failure is *not* new.
- **fixed when:** ten consecutive runs of the unaffected suite produce an identical failing set, and the cause of the non-determinism is named (shared module state across files, a real timer, an unawaited effect, or test-order dependence — `vitest run --sequence.shuffle` and per-file isolation will separate those). Until then, any change using a set diff must build its baseline from a union of at least three runs and say so.

### B-150 — an always-red test absorbs new violations invisibly: the drift check hid three this change had just added
- **state:** open — the masking half is unfixed; the three violations it hid are fixed
- **reported:** 2026-09-23 by this session, while finishing `fleet-structured-notices`. Found only because a *deterministic* failure looked wrong among a flaky union and was re-run on its own instead of being waved through as known debt.
- **measured:** `tests/unit/designDrift.test.ts :: carries no arbitrary font size — the scale is 12/14/16` failed in **3 of 3** runs (B-146 already names it as the one stable failure). Its assertion is `expect(hits(/text-\[\d+px\]/)).toEqual([])` — a set, not a boolean. The set held **7** entries: `FleetWirePanel.tsx` ×4, which `git show HEAD:` confirms were already there, and **3 that were not** — `FleetRestore.tsx:191,202` (0 at HEAD, 2 added by this change) and `Panel.tsx:144` (a new, untracked file). So a change added three violations to a rule, and every regression check passed them: the test's NAME was already on the known-failing list, so a name-level set diff reported "no failure outside the known set" — which was true, and useless.
- **fail direction:** silent accumulation, in the direction that feels safe. An always-red test is a place new violations can be parked forever, and the longer it stays red the more it absorbs. It also makes the debt look static (`4 hits`) while it grows, so nobody sees a reason to act. This is the project's own *"a completeness claim is a measurement, not a summary"* one layer down: the summary here is the test's pass/fail bit, and the measurement is its hit set.
- **also:** the same shape applies to every assertion whose value is a collection — a red `toEqual([])` on a growing list is indistinguishable from a red one on a static list, from the outside.
- **fixed when:** two things, and the second is the one that generalises.
  1. `FleetWirePanel.tsx`'s four `text-[10px]` uses are on the 12/14/16 scale (or the file is explicitly allowlisted in `designDrift.test.ts` with the reason written there), so the test is green and can fail meaningfully again.
  2. A regression check compares an already-failing test's **hit set** against its baseline hit set, not its name — proven by a test that adds one violation to a file while the check is red and asserts the check reports it. Until then, any change touching a file named in a red collection assertion must diff that assertion's own output before and after, and say so.

### B-151 — a consumer's name is in the tracked tree and in a commit message of this PUBLIC repo, and the pre-push gate cannot see it because it scans the range, not the tree
- **state:** open — needs the user's decision; nothing has been scrubbed
- **reported:** 2026-09-23 by this session, running a confidentiality check after archiving an unrelated change. Not found by any gate that ran during that work.
- **measured:** `set-leakscan --tree` over 5603 tracked files returns **3 blocking findings**, all pre-existing and all already pushed: a consumer slug in `openspec/changes/fleet-pm-mode/tasks.md:86` (with several sentences of that consumer's own Hungarian text quoted around it), the same slug in `tests/unit/test_fleet_state.py:420`, and one in the message of commit `5930a07ae275`. A word-boundary `grep` finds **two more the scanner does NOT flag** — `lib/set_orch/usage/accounts.py:9` and `web/tests/unit/fleetUsageBars.test.ts:344` — because both use the bare stem where the registry holds the fuller slug. So the tree carries at least five references and the scanner sees three of them.
- **fail direction:** the worst one for a confidentiality control — quiet, and in the direction of looking safe. Three separate gates passed this work (`pre-commit`, the agent `PreToolUse` hook, and a pre-push `set-leakscan` on the unpushed range, reported `clean (45 files)`), because every one of them scans **what a change ADDS**. That is deliberate and correct for a commit gate, and it means the tree-wide state is checked by nothing on the path a person actually walks. The release-safety rule already names this as check 6 — *the whole tracked tree, not just the diff* — and it is the one check with no command wired into any hook.
- **second, narrower defect:** the name list resolves from the registry's slugs, so a reference to the distinctive **stem** of a consumer's name does not match. A pattern that only catches the full slug will keep missing the form people actually write in prose.
- **already pushed, so state plainly what that means:** this is not revocable by scrubbing. The repository is public and the commits are on `github/main`; a history rewrite is cleanup, not remediation, and it would break every sha reference in this register and in the archived changes. Whether to rewrite is the user's call, not an agent's.
- **CORRECTION 2026-09-23, an hour after this entry was written — the fix below was ordered wrong, and measuring it reversed the priority.** The entry led with a tree gate on pre-push and buried the stem gap third. Measured: `set-leakscan --message` on a consumer name written as a bare stem in prose returns **clean**, while the same name written as its full registered slug is refused — so the bare stem passes the commit gate, the push gate and the agent hook alike. That is the only one of the three items that prevents a FUTURE leak; the other two are cleanup of a name already published and irreversible. And wiring `--tree` into pre-push while it exits 1 would block the next push outright, at **8.45 s** per run, whose first consequence is somebody reaching for `--no-verify` — the outcome this repo's own rule calls worse than having no gate. It would also be an always-red check, which is B-150 in this same register.
- **SECOND DEFECT, found 2026-09-23 by tripping it — the gate table in `release-safety.md` describes three gates, and TWO OF THEM DO NOT EXIST in this checkout.** Measured: `ls .git/hooks/` contains **only `.sample` files**, and `git config core.hooksPath` is unset. So `pre-commit`, `commit-msg` and `pre-push` are all absent — a human committing or pushing here passes no leak gate whatsoever, while the rule's table reads as though three actors are covered. The hooks ship behind `set-leakscan --install-hooks`, which nobody ran in this clone, and nothing anywhere reports that they are missing.
- **THIRD DEFECT, same session, same cause as the miss above: the surviving agent gate is disarmed by chaining.** `set-hook-leakscan` is a `PreToolUse` hook, so it runs **before** the bash command it guards. Its commit gate is `set-leakscan --staged`. Write the commit as `git add X && git commit X` in ONE invocation and the scan runs against the index **as it was before the add** — honestly clean, about nothing. The `push` half fails identically: its scan reads the unpushed range, and the offending commit is created later in the same chain. This is not a scanner bug and no pattern change fixes it; it is a timing property of a pre-execution hook, and it is why an earlier commit in this same session WAS blocked correctly — there the `git add` had run as its own command, so the index was populated when the hook fired.
- **How it was found, which is the only reason it is in writing:** by committing a consumer's registered slug into THIS FILE, in the entry about consumer names reaching public files, and pushing it. Three gates reported clean: the agent hook (empty index), the range scan (commit not yet made), and the git hooks (not installed). The tree scan named it immediately afterwards. Every failure was in the direction of looking clean.
- **fixed when:** the stem item is the one that pays; the other two are optional cleanup, in this order.
  1. **(the valuable one)** The name list also matches the distinctive stem of each registered slug, so a reference written in prose is caught at commit time by the gates that already run. Hold the measured miss in a test, using a fixture slug rather than a real one: the stem written in prose must fire, and a word that merely contains it as a substring (`splitlines` contains the stem of one registered slug) must not — the trap this entry was found through.
  2. If tree coverage is wanted, `set-leakscan --tree` belongs in **CI on `main`**, not on pre-push: 8.45 s taxes nobody there, and a red result is visible without blocking a person mid-push. It must not be wired anywhere blocking until the findings below are zero, or it is a gate whose only use is to be bypassed.
  3. `set-leakscan --install-hooks` has actually been run in this clone, verified by `ls .git/hooks/`, and something reports when the hooks are missing rather than leaving their absence silent. A gate that is documented and not installed is worse than one that is neither, because the document is what people trust.
  4. The agent hook stops trusting a pre-execution scan of post-execution state: either it refuses a command that stages and commits in one invocation, or it scans the working tree the command will commit rather than the index it has not written yet. A test must hold the chained shape, because the unchained one already passes.
  5. The five known references are generalised to neutral names per the confidentiality rule, in a change of their own, and `--tree` comes back clean.
### B-152 — every file-panel read re-runs full agent discovery just to confirm the root is known
- **renumbered:** B-146 → B-152 in the 2026-09-24 merge with origin; the other machine had already published B-146. Commit messages written before the merge cite B-146.
...
- **fixed when:** one file read costs one discovery-free (or one-scan) root check — measured as `_known_root` under 50 ms on a machine with ≥6 000 transcript dirs — with the verdict unchanged for a worktree of a known project and for an unknown directory (both already covered by the `_known_root` tests).
