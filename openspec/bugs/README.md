...
- **fixed when:** three things, and the first is the one that stops accumulation.
  1. `set-leakscan --tree` runs somewhere a person or an agent cannot skip — a pre-push hook step, or a CI job on `main` — so the tree's state is measured rather than inherited.
  2. The name list also matches the distinctive stem of each registered slug, with a test holding the measured miss (`the itline account` must fire, `splitlines` must not — the substring trap this entry was found through).
