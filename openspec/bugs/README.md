...
### B-136 — full-suite runs disagree with themselves: failures appear and vanish between identical runs
- **state:** open
- **reported:** 2026-09-01 by this session, while verifying the tab-badge change.
- **measured:** three full `vitest run` executions (Node v24) of the SAME tree: 5 failed / 1434 passed, then 9 failed / 1430 passed, then 6 failed / 1433 passed. The extra failures (`fleetArrangement` "renders a waiting agent as waiting", `fleetSurface` "selects back"/"costs one click"/"keeps the phase", `srOnlyNotCopied`) all PASS when their file runs in isolation — e.g. `fleetArrangement` 23/23 alone — so the tests are not wrong about the code; the files are polluting each other's state in the shared pool. This is the known failure class the `regression-baseline` skill exists for (the session-end leak assertion).
- **fail direction:** false alarm and false all-clear at once — a green full run does not mean these tests passed meaningfully, and a red one indicts code that is fine. Every full-suite result that disagrees with its file's isolated result is noise until this is fixed.
- **fixed when:** two consecutive full runs of the same tree produce the same failure set, AND each previously-flaky test passes in the full run. Likely suspects: workers sharing a jsdom/document between files, or module-level singletons (`let term = ...` style hooks) not reset in some files' afterEach.
