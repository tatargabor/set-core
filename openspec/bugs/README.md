...
- **state:** open
- **reported:** 2026-09-11 by the user, with a FleetView screenshot: the third docked panel rendered `unnamed · unknown cache · provider unrecorded` over `no input: this process has no session id, so no seat can be resolved`, and the user asked whether the panel is broken or it is "an agent without a terminal".
