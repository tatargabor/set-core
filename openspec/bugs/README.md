...
### B-143 — `set-project init --dry-run` registers the project for real: the local registry write is not covered by the dry-run
- **state:** open
- **reported:** 2026-08-31 by this session, while registering a freshly cloned consumer repo on a new machine.
- **measured:** `set-project init --dry-run` in the clone printed `Project '<name>' registered` and `~/.config/set-core/projects.json` gained the entry in the same second — file mtime `Aug 31 12:45:24`, entry `addedAt: 2026-08-31T10:45:24Z`, both equal to the dry-run's run time; a read of the same file minutes earlier had no such entry (full key list, not a truncated read). The CLI help says `--dry-run: Show what would change without modifying files`. The deploy and the manager call ARE dry (`Would deploy …`, `Would register with manager … (dry run — not sent)`); only the local registry write leaked through.
- **fail direction:** a preview that mutates state. The trust this repo's own record rests on — the deploy preview "proven not to write" — does not hold for the registry path. Outcome here was benign (the entry was wanted anyway), which is exactly what makes it quiet; the same leak on `remove`/`archive` would be less so.
- **fixed when:** `--dry-run` leaves `projects.json` byte-identical — test: snapshot the file, run `set-project init --dry-run` against an unregistered fixture repo, compare bytes.
