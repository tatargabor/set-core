  ...
  return (
    <div className="border border-amber-500/30 bg-amber-500/10 rounded-lg p-4 mb-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="text-amber-200 font-bold text-sm">Orchestration Complete</h3>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-3">
        <button
          ...
          disabled={sending}
          className="px-3 py-1.5 bg-green-600 hover:bg-green-500 text-white text-xs rounded disabled:opacity-50"
        >
          ✅ Accept & Stop
        </button>
        <button
      ...
      <div className="flex gap-2 items-center">
        <input
          ...
          className="flex-1 px-2 py-1 bg-zinc-800 border border-zinc-600 rounded text-xs text-zinc-300 placeholder:text-zinc-600"
        />
        <button
          onClick={() => specPath && sendAction("newspec", specPath)}
          disabled={sending || !specPath}
          className="px-3 py-1.5 bg-purple-600 hover:bg-purple-500 text-white text-xs rounded disabled:opacity-50"
        >
          📋 New Spec
        </button>
      </div>
