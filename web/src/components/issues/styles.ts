...
export const STATE_STYLES: Record<IssueState, { color: string; bg: string; icon: string; label: string }> = {
  ...
  dismissed:          { color: 'text-fg-faint', bg: 'bg-fg-faint/10', icon: '✕',  label: 'Dismissed' },
  muted:              { color: 'text-fg-ghost', bg: 'bg-surface-edge-soft/10', icon: '🔇', label: 'Muted' },
  failed:             { color: 'text-red-400',     bg: 'bg-red-400/10',     icon: '✗',  label: 'Failed' },
  skipped:            { color: 'text-fg-muted', bg: 'bg-fg-muted/10', icon: '→',  label: 'Skipped' },
  cancelled:          { color: 'text-fg-faint', bg: 'bg-fg-faint/10', icon: '⊘',  label: 'Cancelled' },
}

export const SEVERITY_STYLES: Record<string, { color: string; bg: string; label: string }> = {
  unknown:  { color: 'text-fg-muted', bg: 'bg-fg-muted/10', label: '?' },
...
export const MODE_STYLES: Record<string, { color: string; bg: string; label: string }> = {
  ...
  development: { color: 'text-fg-muted', bg: 'bg-fg-faint/10', label: 'DEV' },
}
