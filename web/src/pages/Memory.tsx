...
function TuiBar({ value, max, label, color }: { value: number; max: number; label: string; color: string }) {
  ...
  return (
    <div className="flex items-center gap-3 text-sm">
      <span className="text-fg-muted w-24">{label}</span>
      <span className={color}>{bar}</span>
      <span className="text-fg-normal w-12 text-right">{value}</span>
    </div>
  )
}
...
export default function Memory({ project }: Props) {
  ...
  if (!project) return <div className="flex items-center justify-center h-full text-fg-faint">Select a project</div>
  if (loading) return <div className="p-6 text-fg-faint text-sm">Loading memory stats...</div>
  if (error) return <div className="p-6 text-red-400 text-sm">{error}</div>
  if (!data) return <div className="p-6 text-fg-faint text-sm">No data</div>
      ...
      <section>
        ...
        <div className="bg-surface-panel/50 border border-surface-line px-4 py-3 flex items-center gap-4">
          ...
          {stats.noise_ratio != null && (
            <span className="text-sm text-fg-faint">{Math.round(stats.noise_ratio * 100)}% noise</span>
          )}
        </div>
      </section>
        ...
        <section>
          ...
          <div className="bg-surface-panel/50 border border-surface-line px-4 py-3 space-y-1">
            ...
            {stats.total_retrievals != null && stats.total_retrievals > 0 && (
              <div className="flex items-center gap-3 pt-1 border-t border-surface-line/50 text-sm">
                <span className="text-fg-faint w-24">Retrievals</span>
                <span className="text-fg-muted">{stats.total_retrievals}</span>
              </div>
            )}
          </div>
        </section>
            ...
            {Object.entries(types).sort((a, b) => b[1] - a[1]).map(([type, count]) => {
              ...
              return (
                <div key={type} className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-fg-faint w-10 text-right">{pct}%</span>
                </div>
              )
            })}
          </div>
