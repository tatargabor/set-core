...
**And the repair was put in a file that cannot hold it — measured 2026-08-17.** The fix
above wrote the sentinels into `.claude/skills/openspec-verify-change/SKILL.md`. That file
is GENERATED by the `openspec` CLI: an `openspec update` from 1.1.1 to 1.9.0 rewrote all
ten skills and all ten `/opsx:*` commands wholesale, and every framework-specific line went
with them — the sentinels included. There is no preserve marker and no merge. So the
teaching document is now a framework-owned rule file, and these tests point at it; a
generated file is not a place, it is a rendering.
...
from set_orch.verifier import _VERIFY_RULES_CANDIDATES, _parse_critical_count, _parse_verify_verdict

ROOT = Path(__file__).resolve().parents[2]
#: The framework-owned teaching document. `templates/core/rules/` is the deploy
#: source: `set-project init` copies it into a consumer as
#: `.claude/rules/set-spec-verify-gate.md`, and `verifier.py` resolves whichever
#: name exists at gate time.
RULES = ROOT / "templates/core/rules/spec-verify-gate.md"
SKILL = ROOT / ".claude/skills/openspec-verify-change/SKILL.md"
VERIFIER = ROOT / "lib/set_orch/verifier.py"


def _skill_text() -> str:
    """The teaching document the gate's prompt actually points the agent at."""
...
def test_the_teaching_document_is_one_the_gate_can_actually_find():
    """The prompt names a RESOLVED path, so the file must sit at a name the resolver knows.

    Without this, the rule file and the resolver drift apart the way the rule file and the
    generated skill did: both plausible, neither reachable from the other. Asserted against
    the resolver's own candidate list — not a second copy of the paths.
    """
    assert RULES.is_file(), f"the framework-owned teaching document is missing: {RULES}"

    rel = RULES.relative_to(ROOT).as_posix()
    assert rel in _VERIFY_RULES_CANDIDATES, (
        f"{rel} is not among the paths verifier.py looks for "
        f"({', '.join(_VERIFY_RULES_CANDIDATES)}) — the gate would never name it")


def test_the_generated_skill_is_not_the_place_the_repair_lives():
    """Holds the WRONG pattern, so a future 'tidy-up' back into the skill fails loudly.

    `openspec update` regenerates that file wholesale. Writing the sentinels there looks
    identical to writing them here and survives until the next CLI upgrade — which is the
    exact failure this module's docstring records. A comment asks to be believed; this
    refuses to be reverted.
    """
    if not SKILL.is_file():
        pytest.skip("generated skill not present in this checkout")

    generated_by = re.search(r"generatedBy:\s*\"?([\d.]+)", SKILL.read_text(encoding="utf-8"))
    assert generated_by, (
        "the /opsx:verify skill lost its generatedBy stamp — if it is no longer generated, "
        "this test's premise changed and needs re-measuring, not deleting")
