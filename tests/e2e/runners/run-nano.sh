#!/usr/bin/env bash
# Nano E2E Test Runner
# Smallest possible Next.js scaffold (2 changes) for validating gate enforcement pipeline.
# Tests: e2e autodetect, lint gate, spec verify, profile resolution.
#
# Usage:
#   ./tests/e2e/runners/run-nano.sh                              # Auto-increment
#   ./tests/e2e/runners/run-nano.sh /path/to/dir                 # Use specified dir
#   ./tests/e2e/runners/run-nano.sh --project-dir ~/other-dir    # Override base dir
...
# Generate timestamp-based run ID (e.g., nano-run-20260407-2246)
run_timestamp() {
    date +%Y%m%d-%H%M
}
...
if [[ -n "${1:-}" ]]; then
    ...
    TEST_DIR="$BASE_DIR/nano-run-${RUN_TS}"
    PROJECT_NAME="nano-run-${RUN_TS}"
fi
