#!/usr/bin/env bash
# Micro Blog E2E Test Runner
# Minimal Next.js blog app (5 pages) for validating orchestration pipeline.
# Tests: e2e autodetect, lint gate, spec verify, profile resolution.
#
# Usage:
#   ./tests/e2e/runners/run-micro-blog.sh                              # Auto-increment
#   ./tests/e2e/runners/run-micro-blog.sh /path/to/dir                 # Use specified dir
#   ./tests/e2e/runners/run-micro-blog.sh --project-dir ~/other-dir    # Override base dir

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SCAFFOLD_DIR="$SCRIPT_DIR/../scaffolds/micro-blog"
...
if [[ -n "${1:-}" ]]; then
    ...
    TEST_DIR="$BASE_DIR/micro-blog-run${RUN_NUM}"
    PROJECT_NAME="micro-blog-run${RUN_NUM}"
fi
...
check_existing() {
    if [[ -d "$TEST_DIR/.git" ]]; then
        ...
        echo "  cd $TEST_DIR && set-sentinel --spec docs/spec.md"
        echo ""
        ...
        echo "  set-sentinel --spec docs/spec.md"
        echo ""
...
ATTRS
    ...
    git add -A
    git commit -m "add micro-blog spec"
    git tag v0-spec
