...
show_completion() {
    ...
    echo ""
    info "Archive successful run to git:"
    echo "  cd $TEST_DIR"
    echo "  git remote add archive git@git.setcode.dev:root/craftbrew-run.git 2>/dev/null || true"
    echo "  git push archive main:$PROJECT_NAME"
    echo ""
    info "Cleanup:"
    echo "  rm -rf $TEST_DIR"
